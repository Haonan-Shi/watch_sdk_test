...
static EMp3Res SearchFrameFrom(FIL *pxFil, uint8_t *puBuf, uint32_t uBufLen, uint32_t uStartOfs,
                               const SMp3FrmHdr *pz1stAuFrmHdr, uint32_t *puFoundOfs, uint32_t *puFrmBytes);
static FRESULT SeekRead(FIL *fp, FSIZE_t ofs, void *buff, UINT btr, UINT *br);
...
Mp3Hdl_t Mp3_CreateHandle(const TCHAR *pxFileName, EMp3Res *peRes)
{
    ...
    *peRes = MP3RES_OK;
    ...
    eFr = f_open(&hMp3->xFil, pxFileName, FA_READ);
    if (eFr != FR_OK)
    {
...
EMp3Res Mp3_SetPlayPos(Mp3Hdl_t hMp3, float fPos_ms)
{
    ...
    else // VBR
    {
        ...
        if (uTocIdx == TOC_SIZE)
        {
            ...
            hMp3->uReadOfs = f_size(&hMp3->xFil);
        }
        else
        {
...
EMp3Res Mp3_ReadId3v1(Mp3Hdl_t hMp3, uint8_t **ppuBuf, uint32_t *puLen)
{
    ...
    FSIZE_t uId3v1Ofs = f_size(&hMp3->xFil) - ID3V1_BYTES;
    eFr = SeekRead(&hMp3->xFil, uId3v1Ofs, hMp3->auBuf, ID3V1_BYTES, &uBr);
    if (eFr != FR_OK || uBr != ID3V1_BYTES)
    {
        return MP3RES_FLSEEK_OR_FREAD_ERROR;
    }
                               ...
                               uint32_t *puLen)
{
         ...
         uId3v2Ofs += uId3v2Bytes)
    {
        ...
        else
        {
            ...
            if (eFr != FR_OK || uBr != sizeof(zExtHdr))
            {
                return MP3RES_FLSEEK_OR_FREAD_ERROR;
            }
...
static EMp3Res InitId3v1Info(Mp3Hdl_t hMp3)
{
    ...
    if (f_size(&hMp3->xFil) < ID3V1_BYTES)
    {
    ...
    char acHdr[3];
    UINT uId3v1Ofs = f_size(&hMp3->xFil) - ID3V1_BYTES;
    eFr = SeekRead(&hMp3->xFil, uId3v1Ofs, acHdr, sizeof(acHdr), &uBr);
    if (eFr != FR_OK || uBr != sizeof(acHdr))
    {
        return MP3RES_FLSEEK_OR_FREAD_ERROR;
    }
...
static EMp3Res InitId3v2Info(Mp3Hdl_t hMp3)
{
    ...
    if (f_size(&hMp3->xFil) < ID3V2_HEADER_SIZE)
    {
        hMp3->uId3v2Count = 0;
        hMp3->u1stAuFrmOfs = 0;
        return MP3RES_OK;
    ...
    uint32_t uId3v2Bytes;
    uint32_t uOfs;
    for (uOfs = 0; uOfs < f_size(&hMp3->xFil); uOfs += uId3v2Bytes)
    {
        SId3v2Hdr zHdr;
        eFr = SeekRead(&hMp3->xFil, uOfs, &zHdr, sizeof(SId3v2Hdr), &uBr);
        if (eFr != FR_OK || uBr != sizeof(SId3v2Hdr))
        {
            return MP3RES_FLSEEK_OR_FREAD_ERROR;
        }
