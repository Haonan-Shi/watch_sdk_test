                              ...
                              uint16_t attrib_index, uint16_t offset, uint16_t *length_ptr, uint8_t **pp_value)
{
    ...
    *length_ptr = 0;
    ...
    case TPS_TX_POWER_VALUE_INDEX:
        {
            T_TPS_SERVER_READ_TX_POWER tps_read_tx_power;
            tps_read_tx_power.read_value_index = TPS_READ_TX_POWER_VALUE;
            pfn_tps_cb(conn_handle, cid, GATT_MSG_TPS_SERVER_READ_TX_POWER, (void *)&tps_read_tx_power);

            if (gatt_svc_read_confirm(conn_handle, cid, service_id, attrib_index,
                                      (uint8_t *)&tx_power_value, sizeof(tx_power_value),
                                      sizeof(tx_power_value), APP_RESULT_SUCCESS))
            {
