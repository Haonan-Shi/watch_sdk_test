...
bool ancs_get_hdl_cache(uint8_t conn_id, uint16_t *p_hdl_cache, uint8_t len)
{
    if (conn_id >= ancs_link_num ||
        ...
        len != sizeof(uint16_t) * HDL_ANCS_CACHE_LEN)
    {
        APP_PRINT_ERROR0("ancs_get_hdl_cache: failed");
        return false;
    }
...
{
    if (conn_id >= ancs_link_num ||
        ancs_table[conn_id].disc_state != DISC_ANCS_IDLE ||
        len != sizeof(uint16_t) * HDL_ANCS_CACHE_LEN)
    {
        APP_PRINT_ERROR0("ancs_set_hdl_cache: failed");
        return false;
    }
                       ...
                       uint8_t attribute_ids_len)
{
    ...
    {
        uint16_t offset = 0;
        p_buffer = os_mem_zalloc(OS_MEM_TYPE_DATA, length);
        if (!p_buffer)
        {
            return false;
        }
...
T_CLIENT_ID ancs_add_client(P_FUN_GENERAL_APP_CB app_cb, uint8_t link_num)
{
    ...
    ancs_table = os_mem_zalloc(OS_MEM_TYPE_DATA, size);
