...
# template file
TEMPLATE_FILE="git_version_zephyr.template"
TAG_HEADER=$1
...
# remove tag header
PROJ_VERSION=${FULL_VERSION#"$TAG_HEADER"}
# echo $PROJ_VERSION

# split version to five fields
TAG_VERSION=${PROJ_VERSION}
# echo $TAG_VERSION
...
if [ ! $BUILDER_MAIL ]
then
    BUILDER_MAIL="server@Realtek.com.cn"
fi
...
`cat $TEMPLATE_PATH | sed -e "s/MAJOR_T/$MAJOR/g" \
                     -e "s/MINOR_T/$MINOR/g" \
                     -e "s/PATCHLEVEL_T/$REVISION/g" \
                     -e "s/VERSION_TWEAK_T/$BUILDNUM/g" \
                     -e "s/EXTRAVERSION_T/$CUSTOMER_NAME/g"    \
