...
pack:
windows:
mkfatfs.exe -c Test -s 0x200000 -t fatfs test.bin
linux: 
./mkfatfs -c Test -s 0x200000 -t fatfs test.bin

unpack:
windows:
mkfatfs.exe -u Test -s 0 -t fatfs test.bin
linux:
./mkfatfs -u Test -s 0 -t fatfs test.bin

NOTE:
-p and -b are not supported yet!!!
