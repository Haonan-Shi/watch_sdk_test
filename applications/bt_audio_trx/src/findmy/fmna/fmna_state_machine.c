...
void fmna_rotate_key(void)
{
    ...
    if (m_fmna_current_primary_key.index == cached_next_secondary_key_rotation_index)
    {
        ...
        app_findmy_sched_event_put(NULL, 0, dispatch_update_next_secondary_key_rotation_index);
    }
}
...
{
    FMNA_LOG_INFO("fmna_fmna_pair_evt_disconnected_handler");
...
void fmna_non_owner_connection_timeout_handler(void *p_context)
{
    ...
    int_gpio_msg.type = IO_MSG_TYPE_FINDMY;
    int_gpio_msg.subtype = IO_MSG_FINDMY_DISCONNECT;
    if (false == app_io_msg_send(&int_gpio_msg))
    {
        APP_PRINT_ERROR0("fmna_key_rotation_handler: Send int_gpio_msg failed!");
        return;
...
void fmna_pair_connection_timeout_handler(void *p_context)
{
    ...
    if (false == app_io_msg_send(&int_gpio_msg))
    {
        APP_PRINT_ERROR0("fmna_pair_connection_timeout_handler: Send int_gpio_msg failed!");
        return;
    }
}
...
void separated_ut_timeout_handler(void *p_context)
{
    ...
    int_gpio_msg.type = IO_MSG_TYPE_FINDMY;
    int_gpio_msg.subtype = IO_MSG_FINDMY_UT_START;
    if (false == app_io_msg_send(&int_gpio_msg))
    {
        APP_PRINT_ERROR0("separated_ut_timeout_handler: Send int_gpio_msg failed!");
        return;
    }
}
...
void fmna_persistent_connection_disconnection_timeout_handler(void *p_context)
{
    ...
    int_gpio_msg.type = IO_MSG_TYPE_FINDMY;
    int_gpio_msg.subtype = IO_MSG_FINDMY_RECONNECT_TIMEOUT;
    if (false == app_io_msg_send(&int_gpio_msg))
    {
        APP_PRINT_ERROR0("fmna_persistent_connection_disconnection_timeout_handler: Send int_gpio_msg failed!");
        return;
    }
}
