...
void fmna_motion_detection_start_active_polling(void)
{
    ...
    if (!m_motion_active_polling_ended)
    {
        if (m_motion_sound_count >= SEPARATED_UT_MOTION_MAX_SOUND_COUNT)
        {
            ...
            app_findmy_sched_event_put(NULL, 0, motion_active_poll_duration_timeout_sched_handler);
        }
        else
        {
...
void motion_poll_timer_timeout_handler(void *p_context)
{
    ...
    int_gpio_msg.subtype = IO_MSG_FINDMY_MT;
    if (false == app_io_msg_send(&int_gpio_msg))
    {
        APP_PRINT_ERROR0("motion_backoff_timeout_handler: Send int_gpio_msg failed!");
        return;
    }
}
