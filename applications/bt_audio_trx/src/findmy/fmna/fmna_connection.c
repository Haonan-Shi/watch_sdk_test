...
void fmna_connection_disconnected_handler(uint16_t conn_handle)
{
    FMNA_LOG_INFO("fmna_connection_disconnected_handler: conn_handle 0x%x", conn_handle);
...
void fmna_connection_set_active_ltk(uint8_t new_ltk[GAP_SEC_KEY_LEN])
{
    ...
    if (bt_le_set_dev_ltk(p_entry, false, GAP_SEC_KEY_LEN, m_active_ltk))
    {
...
void fmna_connection_pair_info_restore(void)
{
    ...
    APP_PRINT_INFO1("fmna_connection_pair_info_restore %d", m_is_fmna_paired);
}
