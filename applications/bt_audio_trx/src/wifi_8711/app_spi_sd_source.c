...
bool app_spi_sd_source_open(const char *path, uint32_t *out_size)
{
    ...
    /* bt_audio_trx port: trx's app_fs has no disk-power-down timer (unlike
     * watch), so there is nothing to hold off here - the SD stays powered.
     * Mirrors the commented-out calls in trx's wifi_sdio_tx_test.c. */
...
void app_spi_sd_source_close(void)
{
    ...
    fs_close(&s_sd_file);
    s_sd_open = false;
    /* bt_audio_trx port: see app_spi_sd_source_open() - no disk-power timer. */
