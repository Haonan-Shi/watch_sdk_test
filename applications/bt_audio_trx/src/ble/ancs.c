...
static uint16_t ancs_conn_handle = 0;
static uint32_t incoming_call_notification_uid = 0;
static uint32_t active_call_notification_uid = 0;
static bool ancs_link = false;
                                        ...
                                        T_ANCS_CLIENT_NOTIFICATION_SOURCE *p_notify_src)
{
    ...
    if (p_notify_src->category_id == ANCS_CATEGORY_ID_SOCIAL ||
        p_notify_src->category_id == ANCS_CATEGORY_ID_OTHER ||
        p_notify_src->category_id == ANCS_CATEGORY_ID_INCOMING_CALL ||
        p_notify_src->category_id == ANCS_CATEGORY_ID_EMAIL ||
        p_notify_src->category_id == ANCS_CATEGORY_ID_ACTIVE_CALL)
    {
#endif
        if (p_notify_src->category_id == ANCS_CATEGORY_ID_INCOMING_CALL)
        {
            incoming_call_notification_uid = p_notify_src->notification_uid;
            //refuse or accept incomming call
        }
        else if (p_notify_src->category_id == ANCS_CATEGORY_ID_ACTIVE_CALL)
        {
            active_call_notification_uid = p_notify_src->notification_uid;
        }
...
{
    bool ret = false;
    uint16_t conn_handle = ancs_conn_handle;
    uint32_t noti_uid = incoming_call_notification_uid;
...
bool ancs_end_active_call(void)
{
    bool ret = false;
    uint16_t conn_handle = ancs_conn_handle;
    uint32_t noti_uid = active_call_notification_uid;
    uint8_t action_id = CP_ACTION_ID_NEGATIVE;
