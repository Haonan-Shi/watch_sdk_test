...
#define MFB_INT_TRIGGER_LEVEL_HIGH 1
#define MFB_INT_TRIGGER_LEVEL_LOW 0
...
{
    if (mfb_key.value_report.current_state != GPIO_KEY_PRESSED)
    {
...
static void mfb_work_handler(struct k_work *work)
{
    ...
    if (new_value != mfb_key.level)
    {
        mfb_key.level = new_value;
        mfb_key.value_report.current_state = GPIO_KEY_RELEASED;
        mfb_key.value_report.release_timestamp = k_uptime_get_32();

    }
    else
    {
        mfb_key.value_report.current_state = GPIO_KEY_PRESSED;
        mfb_key.value_report.press_timestamp = k_uptime_get_32();
    }

    if (mfb_key.user_callback != NULL)
    {
        mfb_key.user_callback(&mfb_key.value_report);
    }

    // printk("mfb_work_handler: new_value = %d current_state = %d press_timestamp = %d release_timestamp = %d\n",
