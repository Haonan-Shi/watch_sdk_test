...
typedef struct
{
    ...
    uint16_t opcode;
    uint8_t length;
    uint8_t start;   // 0x00 - 0x01: POUTER_START -->1 , POUTER_STOP --> 0
    uint8_t channel; //0-78
    uint8_t tx_gain; //0x00- 0x02, TX_POWER_TYPE = 0x01, TXGAIN_LEVEL_TYPE = 0x02
    uint8_t tx_power; // TX_POWER_TYPE:     0x00 - 0xFF, signed integer, uint 0.5dbm
    // TXGAIN_LEVEL_TYPE: 0x00 - 0x07, unsigned integer
    // Ignored when type = POUTER_STOP
