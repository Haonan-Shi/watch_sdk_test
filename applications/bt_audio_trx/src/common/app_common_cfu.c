...
#define NOT_READY_OFFSET                197
#define NOT_READY_BIT                   0x40
#define DISABLE_FLASH_BP                7
#else
#define NOT_READY_OFFSET                308
#define NOT_READY_BIT                   0x80
#define DISABLE_FLASH_BP                0
#endif
                                            ...
                                            void *p_void)
{
    ...
    /*cfu packet is 52 bytes. not ready bit offset in image header is NOT_READY_BIT bytes*/
    if (offset == ((NOT_READY_OFFSET / CFU_PACKET_SIZE) * CFU_PACKET_SIZE))
    {
        ...
        *not_ready = (*not_ready) | NOT_READY_BIT;
    }
...
#ifdef CONFIG_SOC_SERIES_RTL87X3G
static uint64_t app_cfu_get_version(void)
{
    T_IMG_HEADER_FORMAT *header_addr = (T_IMG_HEADER_FORMAT *)get_header_addr_by_img_id(IMG_OTA);;

    APP_PRINT_TRACE4("app_cfu_get_version: reserve %d revision %d minor %d major %d",
                     header_addr->git_ver.sub_version._version_reserve,
                     header_addr->git_ver.sub_version._version_revision,
                     header_addr->git_ver.sub_version._version_minor, header_addr->git_ver.sub_version._version_major);
    return header_addr->git_ver.version;
}
#else
...
static void app_cfu_standard_offer_parse(uint8_t *data, FW_UPDATE_OFFER_RESPONSE *rsp)
{
    ...
    APP_PRINT_TRACE3("app_cfu_standard_offer_parse: component_id: %x bank: %d milestine: %d",
                     offer_cmd->component_id, offer_cmd->bank, offer_cmd->milestone);
#if F_APP_TEAMS_CFU_SUPPORT
...
#ifdef CONFIG_SOC_SERIES_RTL87X3G
    uint64_t version = app_cfu_get_version();
#else
...
#ifdef CONFIG_SOC_SERIES_RTL87X3G
        (version >= offer_cmd->version))
    {
        APP_PRINT_ERROR2("app_cfu_standard_offer_parse old fw SOC 0x%llx pack 0x%llx",
                         TRACE_UINT64(version), TRACE_UINT64(offer_cmd->version));
#else
...
#if F_APP_GAMING_CHAT_MIXING_SUPPORT
    legacy_gaming_xfer_start(p_link->bd_addr, LEGACY_GAMING_XFER_1);
#endif
}
...
#ifdef CONFIG_SOC_SERIES_RTL87X3G
    uint64_t version = app_cfu_get_version();
    ver_rsp->component_info[0].version = (uint32_t)(version & 0xFFFFFFFF);
    ver_rsp->component_info[1].version = (uint32_t)((version >> 32) & 0xFFFFFFFF);
#else
...
#ifdef CONFIG_SOC_SERIES_RTL87X3G
            if ((ms_cfu->cur_img_id != IMG_BOOTPATCH) || !ms_cfu->flag.boot_patch_flag)
#endif
            {
                ret = app_cfu_write_data_to_flash(ms_cfu->cur_img_id, ms_cfu->cur_img_offset, write_len,
                                                  content_cmd->data);
            }
...
#ifdef CONFIG_SOC_SERIES_RTL87X3G
                    if (ms_cfu->cur_img_id == IMG_BOOTPATCH)
                    {
                        T_IMG_HEADER_FORMAT *boot_patch_header = 0;
                        boot_patch_header = (T_IMG_HEADER_FORMAT *)app_get_inactive_bank_addr(IMG_BOOTPATCH);
                        APP_PRINT_TRACE2("app_cfu_content_parse: active boot_patch_addr: 0x%x, inactive 0x%x",
                                         get_active_boot_patch_addr(), boot_patch_header->image_base);
                        if (!ms_cfu->flag.boot_patch_flag &&
                            (get_active_boot_patch_addr() != boot_patch_header->image_base))
                        {
                            ms_cfu->flag.boot_patch_flag = 1;
                        }
                    }
#endif
#ifdef USER_DATA1_ADDR
...
#if ((CONFIG_SOC_SERIES_RTL87X3G == 1) || (CONFIG_SOC_SERIES_RTL8773E == 1))
                            base_addr = get_header_addr_by_img_id(IMG_STACKPATCH);
#else
