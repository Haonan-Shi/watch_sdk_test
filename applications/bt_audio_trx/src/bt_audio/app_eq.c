...
void app_eq_enable_info_get(uint8_t eq_mode, T_EQ_ENABLE_INFO *enable_info)
{
    ...
    {
#if F_APP_VOICE_MIC_EQ_SUPPORT || F_APP_RECORD_EQ_SUPPORT
        if (eq_mode == VOICE_MIC_MODE)
        {
            ...
            {
                enable_info->instance = app_src_play_record_eq_instance_get();
                enable_info->idx = app_cfg_nv.eq_idx;
            }
#else
            link = &app_db.br_link[app_hfp_get_active_idx()];
...
bool app_eq_reset_user_eq(T_EQ_TYPE eq_type, uint8_t mode, uint8_t index)
{
    ...
    offset = app_eq_get_user_eq_data_offset(eq_type, mode, index, app_eq_cfg.user_eq_spk_eq_num);
...
#if F_APP_RECORD_EQ_SUPPORT
             || (stream_type == EQ_STREAM_TYPE_RECORD)
#endif
            )
    {
#if F_APP_VOICE_MIC_EQ_SUPPORT || F_APP_RECORD_EQ_SUPPORT
...
#if F_APP_RECORD_EQ_SUPPORT
            || ((T_AUDIO_TRACK_HANDLE)app_audio_track_handle_get(AUDIO_STREAM_TYPE_RECORD) != NULL)
#endif
           )
        {
...
void app_eq_enable_effect(uint8_t eq_mode, uint16_t eq_len)
{
    ...
    if (buf != NULL)
    {
        if (app_db.sw_eq_type == SPK_SW_EQ)
        {
            ...
            {
                link = &app_db.br_link[app_hfp_get_active_idx()];
                link->eq.voice_spk_eq_instance = eq_create(EQ_CONTENT_TYPE_VOICE_OUT, buf, eq_len);

                if (link->eq.voice_spk_eq_instance != NULL)
                {
                    eq_enable(link->eq.voice_spk_eq_instance);
                    audio_track_effect_attach(link->sco.track_handle, link->eq.voice_spk_eq_instance);
                }
            }
            else
#endif
            {
                link = &app_db.br_link[app_a2dp_get_active_idx()];
                link->eq.instance = eq_create(EQ_CONTENT_TYPE_AUDIO, buf, eq_len);

                if (link->eq.instance != NULL)
                {
                    app_eq_audio_eq_enable(&link->eq.instance, &link->eq.audio_eq_enabled);
                    audio_track_effect_attach(link->a2dp.track_handle, link->eq.instance);
                }
            }
        }
        else
        {
            ...
            {
                link = &app_db.br_link[app_hfp_get_active_idx()];
                link->eq.voice_mic_eq_instance = eq_create(EQ_CONTENT_TYPE_VOICE_IN, buf, eq_len);

                if (link->eq.voice_mic_eq_instance != NULL)
                {
                    eq_enable(link->eq.voice_mic_eq_instance);
                    audio_track_effect_attach(link->sco.track_handle, link->eq.voice_mic_eq_instance);
                }
            }
            else
#endif
