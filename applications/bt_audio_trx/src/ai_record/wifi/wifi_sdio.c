...
/* Max times to re-poll RX0_REQ_LEN when RX_REQUEST is asserted but length
 * reads back 0, so a stuck/empty FIFO can't spin the WiFi task forever. */
#define WIFI_SDIO_RX_LEN_MAX_RETRY      8
...
{
    APP_PRINT_INFO0(" [wifi] wifi_sdio_init");
    ...
    sdio_write_byte(&wifi_sdio_func, SDIO_REG_AVAI_BD_NUM_TH_H, 0xb);
    ...
    if (sdio_hisr & SDIO_HISR_RX_REQUEST)
    {
        ...
        do
        {
            ...
            if (rx_len)
            {
                if (rx_len > *size)
                {
                    /* payload larger than caller buffer -- drop to avoid overflow */
                    APP_PRINT_ERROR2("[wifi] rx_len %d > buf %d, drop", rx_len, *size);
                    break;
                }
        ...
        while (++retry < WIFI_SDIO_RX_LEN_MAX_RETRY);
    }
...
int wifi_sdio_write_data(T_WIFI_SDIO_WRITE_QUEUE *write_pkt)
{
    tx_bd_num = wifi_sdio_tx_bd_update();
    {
        /* throttle: this runs on every packet write; cap the log to <=1 per 100 ms */
        static uint32_t last_bd_log_ms = 0;
        uint32_t now_ms = k_uptime_get_32();
        if (now_ms - last_bd_log_ms >= 100)
        {
            last_bd_log_ms = now_ms;
            APP_PRINT_INFO1("tx_bd_num = %d", tx_bd_num);
        }
    }
    if (tx_bd_num == 0)
    ...
    if (write_pkt)
    {
        ...
        /* NOTE: the post-write tx_bd_num re-read was removed. Its result was
         * never consumed here (overwritten by the pre-write read on the next
         * call), so it was one wasted SDIO CMD52 round-trip per packet. */
    }
    else
    {
...
bool wifi_sdio_data_write_queue_fill(uint32_t ip_addr, uint16_t port, uint8_t *data, uint16_t len)
{
    ...
    uint32_t tx_align_size = ((sizeof(TXDESC) + len + 511) & ~511u) + sizeof(sdio_write_pkt->p_next);
...
void wifi_sdio_data_write_queue_flush(uint16_t cnt)
{
    T_WIFI_SDIO_WRITE_QUEUE *sdio_write_pkt;
    {
        /* throttle: called on every flushed packet; cap the log to <=1 per 100 ms */
        static uint32_t last_flush_log_ms = 0;
        uint32_t now_ms = k_uptime_get_32();
        if (now_ms - last_flush_log_ms >= 100)
        {
...
void wifi_sdio_msg_handler(void)
{
    ...
    APP_PRINT_INFO1("sdio read data pkt_len = %d", p_rxdesc->pkt_len);
    if (p_rxdesc->pkt_len)
    {
        ...
        data_read_cb = wifi_sdio_find_data_read_cb(0, 5001);
        if (data_read_cb)
        {
