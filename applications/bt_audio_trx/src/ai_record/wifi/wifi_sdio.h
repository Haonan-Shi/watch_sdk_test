...
#ifndef _WIFI_SDIO_H_
#define _WIFI_SDIO_H_
...
// SDIO_REG_HISR: SDIO Host Interrupt Mask Register
...
typedef uint16_t (*P_FUN_SDIO_DATA_CB)(void *p_data, uint16_t len);
...
typedef struct
{
    ...
    uint16_t    port;
    bool        used;
    P_FUN_SDIO_DATA_CB  sdio_data_read_cb;
} T_WIFI_SDIO_HDL;

typedef struct t_sdio_write_queue
{
    struct t_sdio_write_queue     *p_next;
    TXDESC      tx_desc;
    uint8_t     data[0];
