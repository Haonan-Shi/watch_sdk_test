...
#if (F_APP_BT_HID_DEVICE_SUPPORTT | F_APP_MULTI_2EP_1_BR_1_BLE_HID)
    case HID_PROFILE_MASK:
        {
...
bool linkback_profile_connect_start(uint8_t *bd_addr, uint32_t prof, T_LINKBACK_CONN_PARAM *param)
{
    ...
    case SPP_PROFILE_MASK:
        ret = bt_spp_connect_req(bd_addr, param->server_channel, 1012, 20, param->local_server_chann);
        break;
    ...
    case PAN_PROFILE_MASK:
        ret = bt_pan_connect_req(app_db.factory_addr, bd_addr, BT_PAN_ROLE_PANU, BT_PAN_ROLE_NAP);
        break;
#endif
...
void linkback_profile_disconnect_start(uint8_t *bd_addr, uint32_t profs)
{
    ...
    if (0 != profs)
    {
        ...
        if (profs & PAN_PROFILE_MASK)
        {
            bt_pan_disconnect_req(bd_addr);
        }
    }
    else
    {
