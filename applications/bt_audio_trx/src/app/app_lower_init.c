...
{
#if DT_NODE_HAS_STATUS(DT_NODELABEL(userdata1_region), okay)
#define SECTION_SIZE 1024
#define DTS_USER_DATA1_ADDR DT_REG_ADDR(DT_NODELABEL(userdata1_region))
    ...
    for (uint32_t i = 0; i < 256; i++)
    {
        ...
        memcpy((void *)test_buffer, (void *)(DTS_USER_DATA1_ADDR + i * SECTION_SIZE * 4),
...
static void app_io_resource_cfg(void)
{
    ...
    hw_timer_channel_cfg(BIT2 | BIT3 | BIT4 | BIT5 | BIT6 | BIT7 | BIT8);
}
...
void app_system_lower_init(void)
{
    ...
    {
        DBG_DIRECT("WB OPI psram0 init success!");
    }
    else
    {
        DBG_DIRECT("WB OPI psram0 init fail!");
    }
    ...
    {
        DBG_DIRECT("WB OPI psram1 init success!");
    }
    else
    {
        DBG_DIRECT("WB OPI psram1 init fail!");
    }
