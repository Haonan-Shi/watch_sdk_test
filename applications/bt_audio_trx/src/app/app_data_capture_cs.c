...
void app_data_capture_saiyan_mode_ctl(uint8_t start, uint8_t op_code)
{
    ...
    if (data_capture_saiyan.saiyan_enable != start)
    {
...
#if !defined(CONFIG_SOC_SERIES_RTL8773D) && !defined(CONFIG_SOC_SERIES_RTL87X3G)
        ...
        {
            os_delay(100);
            wdg_reset_with_reason(RESET_ALL, APP_RESET_REASON_DATA_CAPTURE);
        }
        else
        {
            bt_bond_clear();
                                         ...
                                         T_AUDIO_ROUTE_ENTRY   entry[])
{
    ...
    app_audio_route_path_unbind(handle);
    ...
    app_audio_route_path_bind(handle, path);
                ...
                case APP_DATA_CAPTURE_SAIYAN:
                    {
                        ...
                        update_entry[0].io_type = (T_AUDIO_ROUTE_IO_TYPE)sbc_record.ch0_logic_io_type;
                        update_entry[0].endpoint_attr.mic.id = (T_AUDIO_ROUTE_MIC_ID)sbc_record.ch0_mic_sel;
                        update_entry[0].endpoint_attr.mic.type = (T_AUDIO_ROUTE_MIC_TYPE)sbc_record.ch0_mic_type;
                        update_entry[0].endpoint_type = AUDIO_ROUTE_ENDPOINT_MIC;

                        update_entry[1].endpoint_attr.mic.adc_ch = AUDIO_ROUTE_ADC_CHANN1;
                        update_entry[1].io_type = (T_AUDIO_ROUTE_IO_TYPE)sbc_record.ch1_io_type;
                        update_entry[1].endpoint_attr.mic.id = (T_AUDIO_ROUTE_MIC_ID)sbc_record.ch1_mic_sel;
                        update_entry[1].endpoint_attr.mic.type = (T_AUDIO_ROUTE_MIC_TYPE)sbc_record.ch1_mic_type;
