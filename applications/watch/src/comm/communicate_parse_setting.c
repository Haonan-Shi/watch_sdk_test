    ...
    case KEY_PHONE_OS_VERSION:
        {
            if (length == 0x02)
            {
                APP_PRINT_INFO1("Phone OS Version:%d;", pValue[0]); //0x01:ios,0x02:android

                RtkWristbandSys.phone_os_version = pValue[0];

                if (IOS == RtkWristbandSys.phone_os_version)
                {
...
//                }
    ...
    case KEY_FUNCTIONS_REQUEST:
        {
            ...
            if (length == 0x00)
            {
                ...
                L1_send_event(L1SEND_RETURN_RTL87x3B_CONN_INFO, NULL);
    ...
    case KEY_HIDDEN_FUNC_REQUEST:
        ...
        break;
    case KEY_RTL87x3B_MAC_REQUEST:
        {
            if (length == 0x00)
            {
                L1_send_event(L1SEND_RETURN_RTL87x3B_MAC_ADDR, NULL);
            }
        }
        break;
    case KEY_RTL87x3B_STATE_REQUEST:
        {
            if (length == 0x00)
            {
                L1_send_event(L1SEND_RETURN_RTL87x3B_STATE, NULL);
            }
        }
        break;
    case KEY_RTL87x3B_CONNECTED_STATE_REQUEST:
        {
            if (length == 0x00)
            {
                L1_send_event(L1SEND_RETURN_RTL87x3B_CONNECTED_STATE, NULL);
            }
        }
        break;
    case KEY_RTL87x3B_CREATE_CONNECTION_REQUEST:
        {
            if (length == 0x00)
            {
