...
bool L1_send(uint8_t *buf, uint16_t length)
{
    ...
    if (os_msg_recv(l1recv_queue_handle, &event, 100) == true)
    {
...
void L1_receive_data(uint8_t *data, uint16_t length)
{
    ...
    {
        if (data[1] == 0xBA) //Magic word 0xAB 0xBA for protocol ver2
        {
            package_len = (((uint16_t)(data[6])) << 8) + data[7] + 8;
        }
        else
        {
    ...
    if (package_offset == package_len)
    {
        ...
        if (inner_version.value == 0xba)//protocol ver2
        {
            uint16_t rsv_value = (received_buffer[VER2_HEADER_CRC16_HIGH_BYTE_POS] << 8
                                  | received_buffer[VER2_HEADER_CRC16_LOW_BYTE_POS]);
            uint16_t L2_frame_length = received_buffer[VER2_PAYLOAD_LENGTH_LOW_BYTE_POS]
                                       | (received_buffer[VER2_PAYLOAD_LENGTH_HIGH_BYTE_POS] << 8);
        ...
        else //protocol ver1
        {
            ...
            {
                APP_PRINT_INFO0("receive data package & send response");
                WRISTBAND_COMMUNICATE_COMMAND command_id = (WRISTBAND_COMMUNICATE_COMMAND)
                                                           received_buffer[L1_HEADER_SIZE];
                uint8_t first_key = received_buffer[L1_HEADER_SIZE + 2];
                if ((command_id != WATCH_CHATGPT_COMMAND_ID) || (first_key != 0x11))
                {
                    /* send response */
