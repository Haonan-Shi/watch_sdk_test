...
void l1send_task(void *pvParameters)
{
    ...
    os_msg_queue_create(&l1send_queue_handle, "l1sendQ", 0x10, sizeof(uint8_t));
    os_msg_queue_create(&l1recv_queue_handle, "l1recvQ", 0x10, sizeof(uint8_t));
    ...
    while (true)
    {
        if (os_msg_recv(l1send_queue_handle, &event, 0xFFFFFFFF) == true)
        {
            ...
            if (event == L1SEND_AUTO_TEST)
            {
                extern uint32_t auto_test_count;
                APP_PRINT_INFO1("l1send_task auto test: %d", auto_test_count);
            }
#endif
            if (RtkWristbandSys.gap_conn_state == GAP_CONN_STATE_CONNECTED)
            {
                ...
                case L1SEND_RETURN_DEVICE_INFO:
                    {
                        ...
                        psendbuf[15] = KERNEL_VERSION_MAJOR;
                        psendbuf[16] = KERNEL_VERSION_MINOR;
                        psendbuf[17] = 0;//VERSION_REVISION
                        L1_send(psendbuf, 18);
                    }
                    break;
                case L1SEND_RETURN_RTL87x3B_MAC_ADDR:
                    {
                        psendbuf[8] = NOTIFY_COMMAND_ID;            /*command id*/
                        psendbuf[9] = L2_HEADER_VERSION;                /*L2 header version */
                        psendbuf[10] = KEY_RTL87x3B_MAC_ADDRESS_RETURN;   /*first key, */
                        psendbuf[11] = 0;
                        psendbuf[12] = 8;                                /* length  = 1 */

//                        memcpy(psendbuf + 13, RTL87x3B_bd_addr, 6);
//                        psendbuf[19] = RtkWristbandSys.paired_info.paired_flag;
//                        psendbuf[20] = RtkWristbandSys.RTL87x3B_device_status;
                        APP_PRINT_INFO1("[HOWIE TEST] L1SEND_RTL87x3B_MAC_ADDR: %b", \
