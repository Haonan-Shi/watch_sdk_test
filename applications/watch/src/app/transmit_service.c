...
********************************************************************************************************/
static P_FUN_TRANSMIT_SERVER_APP_CB p_fn_transmit_service_cb = NULL;
                                   ...
                                   uint8_t *p_value, P_FUN_EXT_WRITE_IND_POST_PROC *p_write_post_proc)
{
    ...
    *p_write_post_proc = transmit_write_post_callback;
    ...
    if (cause == APP_RESULT_SUCCESS && p_fn_transmit_service_cb)
    {
        p_fn_transmit_service_cb(GATT_MSG_TRANSMIT_SERVER_WRITE, (void *)&callback_data);
    }
    ...
    case TRANSMIT_SVC_TX_DATA_CCCD_INDEX:
        {
            ...
            if (p_fn_transmit_service_cb)
            {
                p_fn_transmit_service_cb(GATT_MSG_TRANSMIT_SERVER_CCCD_UPDATE, (void *)&callback_data);
            }
            break;
        }
    default:
...
/**
  ...
  *
  */
T_SERVER_ID transmit_reg_srv(P_FUN_TRANSMIT_SERVER_APP_CB app_cb)
{
    ...
    p_fn_transmit_service_cb = app_cb;
