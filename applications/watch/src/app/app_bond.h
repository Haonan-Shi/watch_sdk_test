...
/**
 * @brief Remove a bonded device by its app_db.bond_device[] index. If the
 *        device is currently connected, disconnects it first; persists the
 *        bond table to FTL after deletion. Must run on the apptask thread.
 */
...
/**
 * @brief Check if bond device exists at given index
 * @param index bond_device index (0-7)
 * @return true if device exists and has valid address
 */
