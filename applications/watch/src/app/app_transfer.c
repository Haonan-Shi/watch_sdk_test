...
//            }
    ...
    else
    {
        ...
        APP_PRINT_INFO8("app_pop_data_transfer_queue: dt_status %d, active %d, connected_profile 0x%x, rfc_credit %d, rtk_eap_session_status %d, pkt_len:%d, event_id:0x%x, r idx:%d",
                        ...
                        app_db.br_link[app_idx].rtk_eap_session_status,
        ...
        if (dt_queue_ctrl.dt_status == DT_STATUS_IDLE)
        {
            if (dt_queue[dt_queue_ctrl.dt_queue_r_idx].active == CMD_PATH_IAP)
            {
                ...
                {
                    if (app_db.br_link[app_idx].rtk_eap_session_status == DATA_SESSION_OPEN)
                    {
                        if (bt_iap_data_send(app_db.br_link[app_idx].bd_addr, app_db.br_link[app_idx].rtk_eap_session_id,
                                             pkt_ptr,
                                             pkt_len) == true)
                        {
...
//                          }
                    ...
                    else if (app_db.br_link[app_idx].rtk_eap_session_status == DATA_SESSION_CLOSE)
                    {
                        if (app_db.br_link[app_idx].iap_authen_flag)
                        {
                            ...
                            app_db.br_link[app_idx].rtk_eap_session_status = DATA_SESSION_LAUNCH;
...
//                        else
            ...
            else if (dt_queue[dt_queue_ctrl.dt_queue_r_idx].active == CMD_PATH_LE)
            {
                ...
                uint16_t conn_handle = le_get_conn_handle(app_db.le_link[app_idx].conn_id);
                ...
                if (app_db.le_link[app_idx].state == LE_LINK_STATE_CONNECTED)
                {
                    ...
                    {
                        if (transmit_srv_tx_data(conn_handle, cid, pkt_len, pkt_ptr) == true)
                        {
                            dt_queue_ctrl.dt_status = DT_STATUS_ACTIVE;
//                        if (app_cfg_const.enable_embedded_cmd)
                            {
...
static void app_transfer_timeout_cb(uint8_t timer_evt, uint16_t param)
{
    ...
    case APP_TIMER_LAUNCH_APP:
        ...
        if (app_db.br_link[param].rtk_eap_session_status == DATA_SESSION_LAUNCH)
        {
            app_db.br_link[param].rtk_eap_session_status = DATA_SESSION_CLOSE;
        }
        break;
