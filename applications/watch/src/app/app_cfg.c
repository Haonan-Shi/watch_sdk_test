...
T_APP_FUNCTION_CONFIG  app_audio_cfg =
{
    .support_sink = 1,
    .support_local_source = 1,
};

T_APP_AVRCP_FEATURE_CONFIG  app_avrcp_feature_cfg =
{
    .ct_features = 0,
    .tg_features = 0,
};

const T_APP_CFG_CONST app_cfg_default =
{
    ...
    //Miscellaneous
    .device_name_legacy_default = {"BBUWatch"},
    .device_name_le_default = {"BBUWatch"},
...
//    .data_uart_baud_rate = BAUD_RATE_921600,
//#else
...
static bool app_cfg_flash_read(uint32_t addr_offset, void *data, uint32_t len)
{
    ...
    return fmc_flash_nand_read(flash_cur_bank_img_header_load_addr_get(FLASH_IMG_MCUCONFIG) +
                               DEFAULT_HEADER_SIZE + addr_offset,
                               data, len);
#else
    return fmc_flash_nor_read(flash_cur_bank_img_payload_addr_get(FLASH_IMG_MCUCONFIG) + addr_offset,
                              data, len);
#endif
}
...
uint32_t app_cfg_reset(void)
{
    ...
    memset(&app_cfg_nv.pin_code[0], '0', 8);
...
void app_cfg_load(void)
{
    ...
    app_cfg_flash_read(APP_CONFIG_OFFSET, (void *)&sync_word, DATA_SYNC_WORD_LEN);

    if (sync_word == DATA_SYNC_WORD)
    {
        app_cfg_flash_read(APP_CONFIG_OFFSET, (void *)&app_cfg_const, sizeof(T_APP_CFG_CONST));
    }
    else
    {
        memcpy(&app_cfg_const, &app_cfg_default, sizeof(T_APP_CFG_CONST));
    }
    app_dsp_cfg_init(true);
    //20240527 app_dsp_cfg_init(app_cfg_const.normal_apt_support);
    //20240527 app_dsp_cfg_init_gain_level();
