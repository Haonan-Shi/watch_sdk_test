...
static bool app_dsp_cfg_flash_read(uint32_t addr_offset, void *data, uint32_t len)
{
#if CONFIG_APP_NANDBOOT
    //TODO: remove after header file fixed
    extern uint32_t flash_cur_bank_img_header_load_addr_get(FLASH_IMG_ID id);
    return fmc_flash_nand_read(flash_cur_bank_img_header_load_addr_get(FLASH_IMG_DSPCONFIG) +
                               DEFAULT_HEADER_SIZE + addr_offset,
                               data, len);
#else
    return fmc_flash_nor_read(flash_cur_bank_img_payload_addr_get(FLASH_IMG_DSPCONFIG) + addr_offset,
                              data, len);
#endif
}
...
{
    app_dsp_cfg_flash_read(offset, (uint8_t *)p_data, size);
}
...
static void app_dsp_cfg_load_header(void)
{
    ...
    {
        app_dsp_cfg_flash_read(0, &app_dsp_cfg_data->dsp_cfg_header, sizeof(T_APP_DSP_PARAM_R_ONLY));
    }
}
...
static bool app_dsp_cfg_eq_param_load(void)
{
    ...
    app_dsp_cfg_flash_read(data_offset, header, sizeof(T_APP_DSP_EQ_PARAM_HEADER) - 16);
    ...
    app_dsp_cfg_flash_read(data_offset, spk_header,
                           header->eq_spk_application_num * sizeof(T_APP_DSP_EQ_SPK_HEADER));
    ...
    app_dsp_cfg_flash_read(data_offset, mic_header,
                           header->eq_mic_application_num * sizeof(T_APP_DSP_EQ_MIC_HEADER));
    ...
    if (header->voice_eq_applications_num != 0xff)
    {
        ...
        app_dsp_cfg_flash_read(data_offset, voice_eq_header,
                               header->voice_eq_applications_num * sizeof(T_APP_DSP_EQ_SPK_HEADER));
    ...
    app_dsp_cfg_flash_read(data_offset, header->sub_header,
...
static bool app_dsp_cfg_extensible_param_load(void)
{
    ...
    app_dsp_cfg_flash_read(data_offset, (uint8_t *)header,
    ...
    app_dsp_cfg_flash_read(data_offset, header->raw_data, header->extensible_len);
