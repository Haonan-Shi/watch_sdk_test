...
/* Event bus topics for BT connection events (App -> Bridge) */
#define EVENT_BUS_TOPIC_BT_EVT_ALL_TOPIC         "bt_evt/*"
#define EVENT_BUS_TOPIC_BT_EVT_PHONE_CONN        "bt_evt/phone_conn"
#define EVENT_BUS_TOPIC_BT_EVT_PHONE_DISCONN     "bt_evt/phone_disconn"
#define EVENT_BUS_TOPIC_BT_EVT_HEADPHONE_CONN    "bt_evt/headphone_conn"
#define EVENT_BUS_TOPIC_BT_EVT_HEADPHONE_DISCONN "bt_evt/headphone_disconn"
...
/**
 * @brief Toggle the BT radio. enable=true -> STARTUP, enable=false -> IDLE.
 *        Must run on the apptask thread.
 */
void app_bt_policy_set_enabled(bool enable);

/**
 * @brief Initiate a phone (BR/EDR) linkback to the given address.
 *        Must run on the apptask thread.
 */
void app_bt_policy_connect_phone(uint8_t *bd_addr);

/**
 * @brief Initiate a BR/EDR earphone connect to the given address. Cancels any
 *        in-flight earphone linkback first; if another earphone is currently
 *        active, disconnects it before attempting the new connect (caller is
 *        responsible for re-issuing once the disconnect completes).
 *        Must run on the apptask thread.
 */
void app_bt_policy_connect_bredr(uint8_t *bd_addr);

/**
 * @brief Disconnect all profiles for the given BR/EDR address.
 *        Must run on the apptask thread.
 */
