...
const uint8_t hid_descriptor_keyboard_mouse_boot_mode[] =
{
...
static void app_hid_bt_cback(T_BT_EVENT event_type, void *event_buf, uint16_t buf_len)
{
    ...
    case BT_EVENT_HID_CONN_IND:
        {
            p_link = app_find_br_link(param->hid_device_conn_ind.bd_addr);
            if (p_link != NULL)
            {
...
#if F_APP_HID_KEYBOARD_MOUSE_SUPPORT
    bt_hid_descriptor_set(hid_descriptor_keyboard_mouse_boot_mode,
                          sizeof(hid_descriptor_keyboard_mouse_boot_mode));
#endif
