...
/**<  Function pointer used to send event to application from wristband profile. Initiated in bwps_reg_service. */
static P_FUN_WRISTBAND_SERVER_APP_CB pfn_wristband_service_cb = NULL;
...
{
    /*--------------------------BWPS(RTL87x2x wristband private protocol) Service ---------------------------*/
    /* <<Primary Service>>, .. 0*/
    {
...
*/
T_APP_RESULT   bwps_service_attr_read_cb(uint16_t conn_handle, uint16_t cid, T_SERVER_ID service_id,
...
*/
T_APP_RESULT bwps_service_attr_write_cb(uint16_t conn_handle, uint16_t cid, T_SERVER_ID service_id,
                                        ...
                                        P_FUN_EXT_WRITE_IND_POST_PROC *p_write_ind_post_proc)
{
    ...
    case GATT_SRV_BWPS_TX_INDEX:
        ...
        else if (write_type == WRITE_WITHOUT_RESPONSE)
        {
            uint16_t length = ((uint8_t *)(receive_temp))[0];
            uint8_t *p_data = (uint8_t *)receive_temp + 1;
 ...
 * @return none.
*/
void bwps_service_cccd_update_cb(uint16_t conn_handle, uint16_t cid, T_SERVER_ID service_id,
                                 uint16_t index,
                                 uint16_t cccbits)
{
    ...
    T_WRISTBAND_CALLBACK_DATA callback_data;
    ...
    {
        pfn_wristband_service_cb(GATT_MSG_WRISTBAND_SERVER_CCCD_UPDATE, (void *)&callback_data);
    }
}
...
const T_FUN_GATT_EXT_SERVICE_CBS bwps_service_cbs =
{
...
*/
uint8_t wristband_reg_service(P_FUN_WRISTBAND_SERVER_APP_CB app_cb,
                              P_FUN_GATT_EXT_SEND_DATA_CB send_cb)
{
    uint8_t bwps_service_id;
    /* register service callbacks to stack/profile. */
    if (false == gatt_svc_add(&bwps_service_id,
                              (uint8_t *)bwps_service_tbl,
                              sizeof(bwps_service_tbl),
                              &bwps_service_cbs, send_cb))
    {
        APP_PRINT_ERROR0("wristband_reg_service: fail");
        bwps_service_id = 0xff;
        return bwps_service_id;
    }

    pfn_wristband_service_cb = app_cb;
    return bwps_service_id;
}
