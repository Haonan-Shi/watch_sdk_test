...
bool mp_send_msg(T_IO_CONSOLE subtype, void *param_buf)
{
    ...
    return app_send_msg_to_apptask(&msg);
}
...
bool mp_cmd_set_parser(uint8_t *buf, uint32_t len)
{
    ...
    APP_PRINT_INFO2("mp_cmd_set_parser: len %d, buf %b", len, TRACE_BINARY(len, buf));
    ...
    while (mp_cmd_module.rx_count)
    {
        if (mp_cmd_module.rx_buffer[mp_cmd_module.rx_process_offset] == CMD_SYNC_BYTE)
        {
            ...
            if (mp_cmd_module.rx_count >= 7)
            {
                ...
                cmd_id = mp_cmd_module.rx_buffer[temp_index + 1];
                cmd_id |= (mp_cmd_module.rx_buffer[temp_index + 2] << 8);
                ...
                if (mp_cmd_module.rx_count >= (cmd_len + 5))
                {
                    ...
                    if (temp_ptr != NULL)
                    {
                        ...
                        {
                            APP_PRINT_ERROR4("mp_cmd_set_parser: checksum error req %d cmd_id %x, calc 0x%x, rx 0x%x",
                                             rx_seqn, cmd_id, check_sum, temp_ptr[cmd_len + 4]);
...
{
    mp_cmd_module.rx_buffer = os_mem_alloc(OS_MEM_TYPE_DATA, MP_CMD_RX_BUFFER_SIZE);
