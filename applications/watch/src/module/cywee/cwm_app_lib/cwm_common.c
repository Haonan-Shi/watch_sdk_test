...
/**************************************************algo queue**************************************************/
#define QUEUE_MAX_BUF_SIZE (512)/*必须是 2 的整数倍*/
struct queue_t
{
    int32_t head;
    int32_t size;
...
static int32_t queue_get(uint8_t *data)
{
    ...
    uint16_t data_len;
    uint16_t tail = (queue.head >= queue.size) ? (queue.head - queue.size) :
                    (QUEUE_MAX_BUF_SIZE + queue.head - queue.size);
    uint8_t *p = (uint8_t *)&data_len;
    uint16_t des_len = 2;
    while (des_len)
    {
