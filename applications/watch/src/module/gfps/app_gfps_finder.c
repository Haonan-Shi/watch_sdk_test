    ...
    case GFPS_FINDER_UPDATE_RANDOM_WINDOW:
        {
            ...
            {
                uint16_t adv_duration = app_gfps_cfg.gfps_power_off_finder_adv_duration * 100; //uint 10ms
                uint32_t adv_interval = app_gfps_cfg.gfps_power_off_finder_adv_interval;
...
void app_gfps_finder_handle_power_off(void)
{
    ...
    if (app_gfps_cfg.disable_finder_adv_when_power_off == false)
    {
        uint32_t adv_interval = app_gfps_cfg.gfps_power_off_finder_adv_interval;
        gfps_finder_adv_update_adv_interval(adv_interval);
    }
    ...
    if (p_app_gfps_finder->gfps_finder_beacon_conn_id != 0xFF)
    {
        ...
        p_link = app_find_le_link_by_conn_id(p_app_gfps_finder->gfps_finder_beacon_conn_id);

        if (p_link != NULL)
        {
            app_disconnect_le_link(p_link, LE_LOCAL_DISC_CAUSE_GFPS_FINDER);
        }
    }
}
...
void app_gfps_finder_enter_beacon_state(T_GFPS_FINDER_BEACON_STATE beacon_state)
{
    ...
    APP_PRINT_INFO3("app_gfps_finder_enter_beacon_state %d, eik.valid %d, adv_state %d",
...
{
    APP_PRINT_INFO2("app_gfps_finder_reset_conn: temp_modified_eik.valid %d, provisioned %d",
    ...
    if (
        !app_gfps_cfg.disable_finder_adv_when_power_off
        && app_gfps_finder_provisioned())
    {
...
T_GFPS_FINDER_CAUSE app_gfps_finder_cb(T_GFPS_FINDER_CB_DATA *p_data)
{
    ...
    case GFPS_FINDER_EVT_KEY_RECOVERY:
        {
            if (!app_gfps_get_force_enter_pair_mode_state())
            {
