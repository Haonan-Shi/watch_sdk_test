...
#define TONE_GFPS_DULT_FIND           0x8d
...
{
    if (app_audio_tone_play(TONE_GFPS_DULT_FIND, false, false))
    {
...
void app_dult_device_sound_stop(T_SERVER_ID service_id, T_DULT_CALLBACK_DATA *p_callback)
{
    ...
    app_audio_tone_cancel(TONE_GFPS_DULT_FIND, false);
                                 ...
                                 uint16_t buf_len)
{
    ...
    case AUDIO_EVENT_VOICE_PROMPT_STOPPED:
        {
            ...
            if (tone_stopped_idx == app_gfps_cfg.tone_gfps_dult)
            {
                if (keep_ring)
                {
                    app_audio_tone_play(TONE_GFPS_DULT_FIND, false, false);
                }
    ...
    {
        APP_PRINT_ERROR1("app_dult_audio_cback: event_type 0x%04x", event_type);
    }
}
