 ...
 *============================================================================*/
#ifndef _DFU_TRANSPORT_H_
#define _DFU_TRANSPORT_H_
...
#define SPP_PROTOCOL_INFO      0x0016
#define BLE_PROTOCOL_INFO      0x0014

/*ota version num*/

#define BLE_OTA_VERSION     0x5
#define SPP_OTA_VERSION     0x2
...
#define DFU_LENGTH_CP_START_DFU                (1+32) //16bytes ctrl header + 1byte force ota temp + 15bytes reserved
...
#define MAX_IMG_NUM                         (IMG_DFU_MAX - IMG_DFU_FIRST + IMG_USER_DATA_MAX - IMG_USER_DATA_FIRST) //0x15 + 8
...
#define DFU_NOTIFY_LEN_IMG_INFO             (1+1+(2+8)*MAX_IMG_NUM)  //bank_num, img_num, img_id1, img_ver1, ....
...
#define SPP_MAX_BUFFER_SIZE     4096 * 4
#define SPP_DUF_ERASE_SIZE      ERASE_BLOCK_SIZE
...
#define OTA_LENGTH_RWS_INFO             0
#define OTA_LENGTH_OTHER_INFO           0
...
typedef union
{
    ...
    struct
    {
        ...
        uint8_t is_support_normal_ota: 1; //valid only when protocol type 0x0014
        uint8_t is_nand_flash_ota: 1;  /*0: nor flash, 1: nand flash*/
        uint8_t compressed_mode: 1;  /*0: not support, 1: support*/
        uint8_t image_ver_8bytes: 1; /*0: image version 4bytes, 1: image version 8bytes*/
    };
} DEVICE_INFO_MODE;
...
typedef union
{
    ...
    struct
    {
        uint8_t b2b_status: 2;   /*@ref T_RWS_MODE*/
        uint8_t fc_bud_role: 2; /*@ref */
        uint8_t rsvd: 4;
    };
...
typedef struct
{
    ...
    uint16_t mtu_size;
    uint8_t active_bootpatch_banknum; //0: not support dual bank
    uint16_t image_ctrl_header_offset;
    uint16_t compressed_image_ctrl_header_offset;
