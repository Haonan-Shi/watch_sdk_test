...
typedef enum
{
    TRANSIT_CODE_UNBIND             = 0,            //手机端已解绑，请在设备端同步解绑
    TRANSIT_CODE_NET_ERROR          = 1,            //网络错误
    TRANSIT_CODE_DATA_LIMIT         = 2,            //因安全原因限制生码
    TRANSIT_CODE_NOT_SUPPORT        = 3,            //该乘车码不支持
    TRANSIT_CODE_SERVER_ERROR       = 4,            //网络异常，请稍后再试
    TRANSIT_CODE_DEV_ERROR          = 5,            //不该出现的错误
    TRANSIT_CODE_BALANCE_NOT_ENOUGH = 6,            //账户余额不足，请前往手机侧充值
    TRANSIT_CODE_ORDER_OVERDUE      = 7,            //存在行程逾期未补登，请切换到手机端操作
    TRANSIT_CODE_REVOKING           = 8,            //申请退卡中，暂不可用
    TRANSIT_CODE_SENIOR_CARD        = 9,            //您已开通电子敬老卡，请使用电子敬老卡乘车
    TRANSIT_CODE_MONTH_TICKET_REFUNDED = 10,        //当月月票已退，请先充值后再使用
    TRANSIT_CODE_USER_INSURE_EXPIRE = 11,           //保险已到期，请重新购买保险
    TRANSIT_CODE_GO_ANNUAL_REVIEW   = 12,           //卡片已到有效期
    TRANSIT_CODE_GO_CHECK           = 13,           //卡片需要进行人工审核，请等待审核完成
    TRANSIT_CODE_COMMON_ERROR       = 14,           //通用错误
    TRANSIT_BUY_DAY_CARD            = 15,           //请先购买日票后再使用
    TRANSIT_CODE_NEED_RECHARGE      = 16,           //账户余额不足，请切换到手机端进行充值
    TRANSIT_CODE_NOT_FOUND          = 17,           //设备端没有该卡片信息，请切换到手机端使用
 ...
 **/
EXTERNC retval_e alipay_transit_get_card_list_offline(PARAM_OUT alipay_tansit_CardBaseVO_t
                                                      *card_list, PARAM_INOUT uint32_t *len_card_list, PARAM_OUT uint32_t *card_num);
 ...
 *          其他错误                 不应该出现
 **/
EXTERNC retval_e alipay_transit_get_card_list_online(PARAM_OUT alipay_tansit_CardBaseVO_t
                                                     *card_list, PARAM_INOUT uint32_t *len_card_list, PARAM_OUT uint32_t *card_num);
 ...
 **/
EXTERNC retval_e alipay_transit_get_TransitCode(PARAM_IN char *cardNo, PARAM_IN char *cardType,
                                                PARAM_OUT uint8_t *transitcode, PARAM_INOUT uint32_t *len_transitcode,
                                                PARAM_OUT char *error_message, PARAM_IN uint32_t len_error_message);
 ...
 *          其他错误 未获取到上一次卡信息
 */
EXTERNC retval_e alipay_transit_get_the_last_transcode_info(PARAM_OUT alipay_tansit_CardBaseVO_t
                                                            *card_info);
 ...
 * 调用此接口查询具体错误原因, 请根据上方关于错误码的指引提示返回给用户引导界面
 */
 ...
 * 、alipay_transit_get_TransitCode之后可以调用此接口查询具体错误原因，请根据上方关于错误码的指引提示返回给用户引导界面
 */
