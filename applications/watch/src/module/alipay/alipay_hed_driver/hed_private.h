...
#define  HAL_UART_PRINTF_ENABLE   0  //0-Jlink��ӡ��־��1-���ڴ�ӡ��־
...
#define PROTO_I2C_RETRY_NUM                     10//3
#define PROTO_I2C_NAK_NUM                         10//3
...
#define PROTO_I2C_RECEVIE_FRAME_WAIT_TIME     700   //ms   ,  ͨ�ŵ�֡�ȴ�ʱ��:FWT
#define PROTO_I2C_COMM_MUTEX_WAIT_TIME         200   //ms   


#define PROTO_I2C_RECEVIE_POLL_TIME                          1000  //delay us  ֡���ʱ��
...
PERIPHERAL_STRUCT_DEFINE(PERIPHERAL_I2C)
{
    ...
    int32_t (*power_on)(HAL_I2C_PERIPHERAL_STRUCT_POINTER periph);      /* �ϵ�*/
    int32_t (*power_off)(HAL_I2C_PERIPHERAL_STRUCT_POINTER periph);     /*�µ�*/
    int32_t (*lock)(HAL_I2C_PERIPHERAL_STRUCT_POINTER
                    periph);          /* ����(���ö�ռ����)*/
    int32_t (*unlock)(HAL_I2C_PERIPHERAL_STRUCT_POINTER
                      periph);        /* ����(�����ռ����)*/
    se_error_t (*transmit)(HAL_I2C_PERIPHERAL_STRUCT_POINTER periph, uint8_t *data,
                           uint32_t  data_len);  /*�������� */
    int32_t (*receive)(HAL_I2C_PERIPHERAL_STRUCT_POINTER periph, uint8_t *data,
                       uint32_t *data_len);         /* �������� */
    se_error_t (*control)(HAL_I2C_PERIPHERAL_STRUCT_POINTER periph, uint32_t ctrlcode, uint8_t *data,
                          uint32_t  data_len);    /* �շ��������� */
    void *extra;                                               /* ����ͻ��Զ���Ĳ��� */
};
...
typedef struct _peripheral_bus_driver
{
    ...
    se_error_t (*transceive)(peripheral *periph,  uint8_t *sbuf, uint32_t slen, uint8_t  *rbuf,
                             uint32_t *rlen);   /*�����շ�*/
    se_error_t (*reset)(peripheral *periph, uint8_t *rbuf, uint32_t *rlen);         /*��λ */
    se_error_t (*control)(peripheral *periph, uint32_t ctrlcode, uint8_t *sbuf, uint32_t slen,
...
// 改善：用 "format, ##__VA_ARGS__" 替代单独放置 format 和 appending ...
#define LOG_TRACE(level, file, line, func, ...) \
    hal_printf("\n%s - %s(%d) - %s: ", level, file, line, func); \
...
#define LOGV(format, ...) LOG_TRACE("V", __FILE__, __LINE__, __func__, format, ##__VA_ARGS__)
#define LOGD(format, ...) LOG_TRACE("D", __FILE__, __LINE__, __func__, format, ##__VA_ARGS__)
#define LOGI(format, ...) LOG_TRACE("I", __FILE__, __LINE__, __func__, format, ##__VA_ARGS__)
#define LOGW(format, ...) LOG_TRACE("W", __FILE__, __LINE__, __func__, format, ##__VA_ARGS__)
#define LOGE(format, ...) LOG_TRACE("E", __FILE__, __LINE__, __func__, format, ##__VA_ARGS__)
#define LOGF(format, ...) LOG_TRACE("F", __FILE__, __LINE__, __func__, format, ##__VA_ARGS__)
...
se_error_t apdu_scp_init_update(const uint8_t *in_buf, uint32_t in_buf_len, uint8_t *out_buf,
                                uint32_t *out_buf_len);
...
se_error_t apdu_read_file(uint32_t offset, uint32_t expect_len, uint8_t *out_buf,
                          uint32_t *out_buf_len);
...
se_error_t tpdu_execute_no_response(iso_command_apdu_t *command, uint8_t *input,
                                    uint32_t input_len);
...
extern se_error_t proto_i2c_transceive(peripheral *periph, uint8_t *sbuf,    uint32_t  slen,
                                       uint8_t *rbuf, uint32_t *rlen);
extern se_error_t proto_i2c_reset(peripheral *periph, uint8_t *rbuf, uint32_t *rlen);
extern se_error_t proto_i2c_control(peripheral *periph, uint32_t ctrlcode, uint8_t *sbuf,
                                    uint32_t slen, uint8_t  *rbuf, uint32_t *rlen);
