...
#define GATT_MSG_AIS_SERVER_READ           0x00
#define GATT_MSG_AIS_SERVER_WRTIE          0x01
#define GATT_MSG_AIS_SERVER_CCCD           0x02
...
/**
 * @brief P_FUN_AIS_SERVER_APP_CB AIS Server Callback Function Point Definition Function
 *        pointer used in AIS server module, to send events to specific server module.
 */
typedef uint16_t (*P_FUN_AIS_SERVER_APP_CB)(uint8_t type, void *p_data);

bool ais_set_parameter(T_AIS_PARAM_TYPE param_type, uint16_t len, void *p_value);

T_SERVER_ID ais_reg_srv(P_FUN_AIS_SERVER_APP_CB app_cb);
