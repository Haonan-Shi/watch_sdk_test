...
#define FMNA_LOG_ERROR(fmt,...)                 DBG_DIRECT(fmt, ##__VA_ARGS__)//DBG_BUFFER(SUBTYPE_FORMAT, MODULE_APP, LEVEL_ERROR, "FMNA!!!"fmt, ARG_COUNT(__VA_ARGS__),__VA_ARGS__)
#define FMNA_LOG_WARNING(fmt,...)               DBG_DIRECT(fmt, ##__VA_ARGS__)//DBG_BUFFER(SUBTYPE_FORMAT, MODULE_APP, LEVEL_WARN, "FMNA!!*"fmt, ARG_COUNT(__VA_ARGS__),__VA_ARGS__)
#define FMNA_LOG_INFO(fmt,...)                  DBG_DIRECT(fmt, ##__VA_ARGS__)//DBG_BUFFER(SUBTYPE_FORMAT, MODULE_APP, LEVEL_INFO, "FMNA!**"fmt, ARG_COUNT(__VA_ARGS__),__VA_ARGS__)
#define FMNA_LOG_DEBUG(fmt,...)                 DBG_DIRECT(fmt, ##__VA_ARGS__)//DBG_BUFFER(SUBTYPE_FORMAT, MODULE_APP, LEVEL_TRACE, "FMNA"fmt, ARG_COUNT(__VA_ARGS__),__VA_ARGS__)
...
#define MAX_SUPPORTED_CONNECTIONS               CONFIG_FINDMY_MAX_LINKS
...
#if (USER_DATA2_ADDR == 0x00000000)
#error "USER_DATA2_ADDR cannot be 0x00000000. Please set it to a valid address."
#endif
#define APPLE_AUTH_TOKEN_SAVE_ADDRESS           USER_DATA2_ADDR
#define APPLE_AUTH_TOKEN_BACKUP_ADDRESS         (USER_DATA2_ADDR + 0x1000)
