 ...
 * @retval    T_APP_RESULT
 */
static T_APP_RESULT fmna_ais_srv_cb(uint8_t type, void *p_data)
{
    ...
    case GATT_MSG_AIS_SERVER_READ:
        {
            ...
            {
                uint8_t accessory_category[ACC_CATEGORY_MAX_LEN] = {0};
                accessory_category[0] = ACCESSORY_CATEGORY;
 ...
 * @retval    T_APP_RESULT
 */
static T_APP_RESULT fmna_fns_srv_cb(uint8_t type, void *p_data)
{
    ...
    FMNA_LOG_INFO("fmna_fns_srv_cb: type %d, opcode %d, valid %d, conn_id %d", type,
                  p_srv_data->msg_data.write.opcode, fmna_connection_is_valid_connection(conn_id), conn_id);

    if (type == GATT_MSG_FNS_SERVER_WRTIE)
    {
        //TBD
//        if (fmna_connection_is_valid_connection(conn_id))
    ...
    else if (type == GATT_MSG_FNS_SERVER_CCCD)
    {
 ...
 * @retval    T_APP_RESULT
 */
static T_APP_RESULT fmna_tps_srv_cb(uint16_t conn_handle, uint16_t cid, uint8_t type, void *p_data)
{
    T_APP_RESULT app_result = APP_RESULT_SUCCESS;
    T_TPS_SERVER_READ_TX_POWER *p_tps_cb_data = (T_TPS_SERVER_READ_TX_POWER *)p_data;

    APP_PRINT_INFO3("fmna_tps_srv_cb: conn_handle 0x%x, cid 0x%x, type %d", conn_handle, cid, type);

    switch (type)
    {
    case GATT_MSG_TPS_SERVER_READ_TX_POWER:
        {
            if (p_tps_cb_data->read_value_index == TPS_READ_TX_POWER_VALUE)
            {
 ...
 */
#if CONFIG_FINDMY_USE_UARP
static T_APP_RESULT fmna_fwus_srv_cb(uint8_t type, void *p_data)
{
    T_APP_RESULT result = APP_RESULT_SUCCESS;
    fmna_ret_code_t ret_code = FMNA_SUCCESS;
    ...
    if (fmna_connection_is_valid_connection(conn_id))
    {
        if (type == GATT_MSG_FWUS_SERVER_WRTIE)
        {
            switch (p_srv_data->msg_data.write.opcode)
            {
        ...
        else if (type == GATT_MSG_FWUS_SERVER_CCCD)
        {
            switch (p_srv_data->msg_data.nofity_indicate_update.notification_indification_index)
            {
...
void fmna_data_send_cb(T_EXT_SEND_DATA_RESULT result)
{
    if (result.cause == GAP_SUCCESS)
    {
                       ...
                       sizeof(fmna_service_current_extended_packet_tx)))
        {
            ...
            {
                APP_PRINT_INFO0("app_ble_service common_cb: UARP Packet has finished being sent");
        ...
        {
            APP_PRINT_INFO0("app_ble_service common_cb: sending another indication");
...
{
#if CONFIG_FINDMY_USE_UARP
    fwus_srv_id = fwus_reg_srv(fmna_fwus_srv_cb, fmna_data_send_cb);
#endif
    ais_srv_id = ais_reg_srv(fmna_ais_srv_cb);
    fns_srv_id = fns_reg_srv(fmna_fns_srv_cb, fmna_data_send_cb);
    tps_srv_id = tps_reg_srv(fmna_tps_srv_cb);
}

void fmna_gatt_platform_init(void)
{
    os_msg_queue_create(&app_findmy_queue_handle, "findmy_msg", 0x20, sizeof(T_FINDMY_BLE_INDICATION));
    fmna_gap_init();
...
// TODO: get cridits;fmna_gatt_platform_send_indication_busy
    ...
    else
    {
        ...
        indication_msg.buf = os_mem_alloc(RAM_TYPE_DATA_ON, length);
...
void fmna_on_disconnect(uint8_t conn_id)
{
    ...
    app_findmy_disconnect();
}

bool fmna_link_by_conn_id(uint8_t conn_id)
{
    return (recent_conn_handle == conn_id);
}
