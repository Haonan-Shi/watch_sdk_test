...
uint8_t unpair_timer_id = 0;
uint8_t timer_handle_unpair = 0;
typedef enum
{
    APP_FINDMY_UNPAIR             = 0x00,
} T_APP_FINDMY_UNPAIR_TIMER;
...
void app_findmy_unpair_timeout_cb(uint8_t timer_evt, uint16_t param)
{
    switch (timer_evt)
    {
    case APP_FINDMY_UNPAIR:
        {
            fmna_connection_set_is_fmna_paired(false);
            app_stop_timer(&timer_handle_unpair);
        }
        break;
                                          ...
                                          uint16_t length)
{
    ...
    case FMNA_SERVICE_OPCODE_UNPAIR:
        app_timer_reg_cb(app_findmy_unpair_timeout_cb, &unpair_timer_id);
        ...
        else
        {
            ...
            response_status = RESPONSE_STATUS_SUCCESS;
            app_start_timer(&timer_handle_unpair, "findmy_unpair", unpair_timer_id, APP_FINDMY_UNPAIR, 0, false,
                            5000);
        }
        break;
