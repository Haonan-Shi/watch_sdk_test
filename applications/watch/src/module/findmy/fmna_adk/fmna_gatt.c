...
static T_EVENT_BUS_SUBSCRIBER_HANDLE fmna_gatt_send_packet_ext_indication_async_handle;
static T_EVENT_BUS_SUBSCRIBER_HANDLE fmna_gatt_send_next_packet_async_handle;
static int32_t fmna_gatt_dispatch_send_packet_extension_indication_handler_wrapper(
    T_EVENT_BUS_EVENT_DATA *event_data);
static int32_t fmna_gatt_dispatch_send_next_packet_handler_wrapper(T_EVENT_BUS_EVENT_DATA
                                                                   *event_data);
...
static int32_t fmna_gatt_dispatch_send_packet_extension_indication_handler_wrapper(
    T_EVENT_BUS_EVENT_DATA *event_data)
{
    fmna_gatt_dispatch_send_packet_extension_indication_handler(event_data->data, event_data->data_len);
    return 0;
}

void fmna_gatt_dispatch_send_packet_extension_indication(void)
{
    app_findmy_sched_event_put(NULL, 0, EVENT_BUS_TOPIC_FINDMY_EVENT_GATT_SEND_NEXT_PACKET);
}
...
static void fmna_gatt_event_bus_init(void)
{
    event_bus_topic_register(EVENT_BUS_TOPIC_FINDMY_EVENT_GATT_SEND_PACKET_EXT_INDICATION);
    event_bus_topic_register(EVENT_BUS_TOPIC_FINDMY_EVENT_GATT_SEND_NEXT_PACKET);

    event_bus_subscribe_async(&fmna_gatt_send_packet_ext_indication_async_handle,
                              EVENT_BUS_TOPIC_FINDMY_EVENT_GATT_SEND_PACKET_EXT_INDICATION,
                              event_bus_async_send_to_apptask,
                              NULL,
                              fmna_gatt_dispatch_send_packet_extension_indication_handler_wrapper);
    event_bus_subscribe_async(&fmna_gatt_send_next_packet_async_handle,
                              EVENT_BUS_TOPIC_FINDMY_EVENT_GATT_SEND_NEXT_PACKET,
                              event_bus_async_send_to_apptask,
                              NULL,
                              fmna_gatt_dispatch_send_next_packet_handler_wrapper);
}

void fmna_gatt_init(void)
{
    fmna_gatt_platform_init();
    fmna_gatt_event_bus_init();
}
...
static int32_t fmna_gatt_dispatch_send_next_packet_handler_wrapper(T_EVENT_BUS_EVENT_DATA
                                                                   *event_data)
{
    fmna_gatt_dispatch_send_next_packet_handler(event_data->data, event_data->data_len);
    return 0;
}
...
{
    app_findmy_sched_event_put(NULL, 0, EVENT_BUS_TOPIC_FINDMY_EVENT_GATT_SEND_PACKET_EXT_INDICATION);
}
