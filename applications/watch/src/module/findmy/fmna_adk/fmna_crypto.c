...
void fmna_crypto_init(void)
{
    ...
    APP_PRINT_INFO1("fmna_crypto_init: m_software_auth_token preview %b", TRACE_BINARY(16,
                    token_preview));//FMNA_LOG_INFO

    fmc_flash_nor_read(SOFTWARE_AUTH_UUID_ADDR, (uint8_t *)m_mfi_struct.m_software_auth_uuid,
                       SOFTWARE_AUTH_UUID_BLEN);
    APP_PRINT_INFO1("fmna_crypto_init: m_software_auth_uuid %b", TRACE_BINARY(16,
                                                                              m_mfi_struct.m_software_auth_uuid));//FMNA_LOG_INFO
...
{
    FMNA_LOG_INFO("fmna_log_serial_number: Serial Number %s", m_serial_number);//TRACE_STRING
}
