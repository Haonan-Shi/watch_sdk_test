...
static T_EVENT_BUS_SUBSCRIBER_HANDLE findmy_motion_active_poll_duration_timeout_async_handle;
static int32_t motion_active_poll_duration_timeout_sched_handler_wrapper(
...
void fmna_motion_detection_init(void)
{
    ...
    event_bus_topic_register(EVENT_BUS_TOPIC_FINDMY_EVENT_MOTION_ACTIVE_POLL_DURATION_TIMEOUT);
    event_bus_subscribe_async(&findmy_motion_active_poll_duration_timeout_async_handle,
                              EVENT_BUS_TOPIC_FINDMY_EVENT_MOTION_ACTIVE_POLL_DURATION_TIMEOUT,
                              event_bus_async_send_to_apptask,
                              NULL,
                              motion_active_poll_duration_timeout_sched_handler_wrapper);
...
static int32_t motion_active_poll_duration_timeout_sched_handler_wrapper(
    T_EVENT_BUS_EVENT_DATA *event_data)
{
    motion_active_poll_duration_timeout_sched_handler(event_data->data, event_data->data_len);
    return 0;
}
...
void fmna_motion_detection_start_active_polling(void)
{
    ...
    if (!m_motion_active_polling_ended)
    {
        if (m_motion_sound_count >= SEPARATED_UT_MOTION_MAX_SOUND_COUNT)
        {
            ...
            app_findmy_sched_event_put(NULL, 0,
                                       EVENT_BUS_TOPIC_FINDMY_EVENT_MOTION_ACTIVE_POLL_DURATION_TIMEOUT);
        }
        else
        {
...
void motion_poll_timer_timeout_handler(void *p_context)
{
    ...
    if (event_bus_publish(EVENT_BUS_TOPIC_FINDMY_STATE_MACHINE_MT, NULL, 0) != 0)
    {
        APP_PRINT_ERROR0("motion_poll_timer_timeout_handler: Publish event bus message failed!");
        return;
    }
}
...
void motion_backoff_timeout_handler(void *p_context)
{
    ...
    {
        APP_PRINT_ERROR0("motion_backoff_timeout_handler: Publish event bus message failed!");
        return;
    }
}
