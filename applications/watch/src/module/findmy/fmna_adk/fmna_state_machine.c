...
static int32_t dispatch_fmna_sm_event_handler_wrapper(T_EVENT_BUS_EVENT_DATA *event_data);
static int32_t dispatch_update_next_secondary_key_rotation_index_wrapper(
    T_EVENT_BUS_EVENT_DATA *event_data);
static int32_t dispatch_set_next_secondary_key_rotation_index_handler_wrapper(
    T_EVENT_BUS_EVENT_DATA *event_data);
...
static T_EVENT_BUS_SUBSCRIBER_HANDLE fmna_sm_set_next_2nd_key_event_bus_handle;
static T_EVENT_BUS_SUBSCRIBER_HANDLE fmna_sm_update_next_2nd_key_event_bus_handle;
static T_EVENT_BUS_SUBSCRIBER_HANDLE fmna_sm_state_update_event_bus_handle;
...
void fmna_state_machine_set_next_keyroll_ms(uint32_t next_keyroll_ms)
{
    ...
    FMNA_LOG_INFO("fmna_state_machine_set_next_keyroll_ms: start rotation timer P1 %d",
                  next_keyroll_ticks);
...
static int32_t dispatch_set_next_secondary_key_rotation_index_handler_wrapper(
    T_EVENT_BUS_EVENT_DATA *event_data)
{
    dispatch_set_next_secondary_key_rotation_index_handler(event_data->data, event_data->data_len);
    return 0;
}
...
void fmna_state_machine_set_next_secondary_key_rotation_index(uint16_t conn_handle, uint32_t index)
{
    ...
    app_findmy_sched_event_put(&conn_handle, sizeof(conn_handle),
                               EVENT_BUS_TOPIC_FINDMY_EVENT_STATEMACHINE_SET_NEXT_2ND_KEY);
}
...
static int32_t dispatch_update_next_secondary_key_rotation_index_wrapper(
    T_EVENT_BUS_EVENT_DATA *event_data)
{
    dispatch_update_next_secondary_key_rotation_index(event_data->data, event_data->data_len);
    return 0;
}
...
void fmna_rotate_key(void)
{
    ...
    if (m_fmna_current_primary_key.index == cached_next_secondary_key_rotation_index)
    {
        ...
        app_findmy_sched_event_put(NULL, 0, EVENT_BUS_TOPIC_FINDMY_EVENT_STATEMACHINE_UPDATE_NEXT_2ND_KEY);
    }
}
...
static void fmna_state_machine_event_bus_init(void)
{
    event_bus_topic_register(EVENT_BUS_TOPIC_FINDMY_EVENT_STATEMACHINE_SET_NEXT_2ND_KEY);
    event_bus_topic_register(EVENT_BUS_TOPIC_FINDMY_EVENT_STATEMACHINE_UPDATE_NEXT_2ND_KEY);
    event_bus_topic_register(EVENT_BUS_TOPIC_FINDMY_EVENT_STATEMACHINE_STATE_UPDATE);

    event_bus_subscribe_async(&fmna_sm_set_next_2nd_key_event_bus_handle,
                              EVENT_BUS_TOPIC_FINDMY_EVENT_STATEMACHINE_SET_NEXT_2ND_KEY,
                              event_bus_async_send_to_apptask,
                              NULL,
                              dispatch_set_next_secondary_key_rotation_index_handler_wrapper);

    event_bus_subscribe_async(&fmna_sm_update_next_2nd_key_event_bus_handle,
                              EVENT_BUS_TOPIC_FINDMY_EVENT_STATEMACHINE_UPDATE_NEXT_2ND_KEY,
                              event_bus_async_send_to_apptask,
                              NULL,
                              dispatch_update_next_secondary_key_rotation_index_wrapper);

    event_bus_subscribe_async(&fmna_sm_state_update_event_bus_handle,
                              EVENT_BUS_TOPIC_FINDMY_EVENT_STATEMACHINE_STATE_UPDATE,
                              event_bus_async_send_to_apptask,
                              NULL,
                              dispatch_fmna_sm_event_handler_wrapper);
...
void fmna_state_machine_init(void)
{
    ...
    fmna_state_machine_event_bus_init();
}
...
static int32_t dispatch_fmna_sm_event_handler_wrapper(T_EVENT_BUS_EVENT_DATA *event_data)
{
    dispatch_fmna_sm_event_handler(event_data->data, event_data->data_len);
    return 0;
}

void fmna_state_machine_dispatch_event(FMNA_SM_Event_t fmna_evt)
{
    app_findmy_sched_event_put(&fmna_evt, sizeof(fmna_evt),
                               EVENT_BUS_TOPIC_FINDMY_EVENT_STATEMACHINE_STATE_UPDATE);
}
...
uint32_t fmna_boot_evt_boot_handler(FMNA_SM_Event_t fmna_evt, void *p_context)
{
    ...
    APP_PRINT_INFO1("fmna_boot_evt_boot_handler: current secondary key %b",
                    TRACE_BINARY(4, m_fmna_current_secondary_key.public_key));//FMNA_LOG_INFO
    APP_PRINT_INFO1("fmna_boot_evt_boot_handler: current primary key %b",
                    TRACE_BINARY(4, m_fmna_current_primary_key.public_key));//FMNA_LOG_INFO
                                                                    ...
                                                                    void  *p_context)
{
    ...
    p_fmna_send_pairing_data = fmna_malloc(SEND_PAIRING_DATA, sizeof(fmna_send_pairing_data_t));
    if (p_fmna_send_pairing_data == NULL)
    {
        APP_PRINT_ERROR0("fmna_unpaired_connecting_evt_fmna_pairing_initiate_handler: malloc failed!");
        return FMNA_SM_STATUS_CRYPTO_FAIL;
    }

#if HARDCODED_PAIRING_ENABLED
    memset(p_fmna_send_pairing_data, 0xFF, sizeof(fmna_send_pairing_data_t));
#else
    ...
    else
    {
        ...
        fmna_gatt_send_indication(fmna_gatt_get_most_recent_conn_handle(),
                                  ...
                                  p_fmna_send_pairing_data,
                                  sizeof(fmna_send_pairing_data_t));
    }

    fmna_free(SEND_PAIRING_DATA);
                                                          ...
                                                          void *p_context)
{
    ...
    p_fmna_send_pairing_status = fmna_malloc(SEND_PAIRING_STATUS, sizeof(fmna_send_pairing_status_t));
    if (p_fmna_send_pairing_status == NULL)
    {
        APP_PRINT_ERROR0("fmna_fmna_pair_evt_fmna_pairing_finalize_handler: malloc failed!");
        return FMNA_SM_STATUS_CRYPTO_FAIL;
    }

#if HARDCODED_PAIRING_ENABLED
    memset(p_fmna_send_pairing_status, 0xFF, sizeof(fmna_send_pairing_status_t));
#else
    ...
    else
    {
        ...
        fmna_gatt_send_indication(fmna_gatt_get_most_recent_conn_handle(),
                                  ...
                                  p_fmna_send_pairing_status,
                                  sizeof(fmna_send_pairing_status_t));
    }
    return sm_ret;
}
#else //HARDCODED_PAIRING_ENABLED

    fmna_free(FINALIZE_PAIRING_DATA);
...
void fmna_evt_handler(FMNA_SM_Event_t fmna_evt, void *p_context)
{
    ...
    FMNA_LOG_INFO("fmna_evt_handler | Current State %s Event %s",
                  fmna_sm_state_strings[m_fmna_state], //TRACE_STRING
                  fmna_sm_event_strings[fmna_evt]);   //TRACE_STRING
    ...
    for (uint16_t i = 0; i < list_len; i++)
    {
            ...
            (fmna_sm_handlers[m_fmna_state].p_array_handlers[i].fmna_evt == fmna_evt))
        {
            ...
            FMNA_LOG_INFO("fmna_evt_handler: Zstate %s||%d, event %s", fmna_sm_state_strings[m_fmna_state], i,
                          fmna_sm_event_strings[fmna_evt]);
            FMNA_LOG_INFO("fmna_evt_handler: Zret %#x, state_success %d", ret_code,
                ...
                (fmna_sm_handlers[m_fmna_state].p_array_handlers[i].fmna_state_success != FMNA_SM_NOCHANGE))
            {
                FMNA_LOG_INFO("fmna_evt_handler: successful transition old %s new %s",
                              ...
                              fmna_sm_state_strings[fmna_sm_handlers[m_fmna_state].p_array_handlers[i].fmna_state_success]);//TRACE_STRING
                     ...
                     (fmna_sm_handlers[m_fmna_state].p_array_handlers[i].fmna_state_fail != FMNA_SM_NOCHANGE))
            {
                FMNA_LOG_INFO("fmna_evt_handler: fail transition old %s new %s",
                              ...
                              fmna_sm_state_strings[fmna_sm_handlers[m_fmna_state].p_array_handlers[i].fmna_state_fail]);//TRACE_STRING
    ...
    if (evtHandled == false)
    {
        FMNA_LOG_INFO("fmna_evt_handler: Event Handler %s was not executed because it is not in the state: %s",
                      fmna_sm_event_strings[fmna_evt], fmna_sm_state_strings[m_fmna_state]);//TRACE_STRING
    }
}
...
static void fmna_key_rotation_handler(void *p_context)
{
    if (event_bus_publish(EVENT_BUS_TOPIC_FINDMY_STATE_MACHINE_KEYROLL, NULL, 0) != 0)
    {
        APP_PRINT_ERROR0("fmna_key_rotation_handler: Publish keyroll event failed!");
        return;
    }
}
...
void fmna_non_owner_connection_timeout_handler(void *p_context)
{
    ...
    if (event_bus_publish(EVENT_BUS_TOPIC_FINDMY_STATE_MACHINE_DISCONNECT, NULL, 0) != 0)
    {
        APP_PRINT_ERROR0("fmna_non_owner_connection_timeout_handler: Publish non-owner timeout event failed!");
        return;
    }
}
...
void fmna_pair_connection_timeout_handler(void *p_context)
{
    ...
    if (event_bus_publish(EVENT_BUS_TOPIC_FINDMY_STATE_MACHINE_DISCONNECT, NULL, 0) != 0)
    {
        APP_PRINT_ERROR0("fmna_pair_connection_timeout_handler: Publish disconnect event failed!");
        return;
    }
}
...
void separated_ut_timeout_handler(void *p_context)
{
    ...
    if (event_bus_publish(EVENT_BUS_TOPIC_FINDMY_STATE_MACHINE_UT_START, NULL, 0) != 0)
    {
        APP_PRINT_ERROR0("separated_ut_timeout_handler: Publish UT start event failed!");
        return;
    }
}
...
{
    if (event_bus_publish(EVENT_BUS_TOPIC_FINDMY_STATE_MACHINE_RECONNECT_TIMEOUT, NULL, 0) != 0)
    {
        APP_PRINT_ERROR0("fmna_persistent_connection_disconnection_timeout_handler: Publish reconnect timeout event failed!");
        return;
    }
}
...
void fmna_state_machine_handle_msg(const char *topic)
{
    APP_PRINT_INFO1("fmna_state_machine_handle_msg: topic %s", TRACE_STRING(topic));
    ...
    else if (strcmp(topic, EVENT_BUS_TOPIC_FINDMY_STATE_MACHINE_DISCONNECT) == 0)
    {
        fmna_connection_disconnect_this();
    }
    else if (strcmp(topic, EVENT_BUS_TOPIC_FINDMY_STATE_MACHINE_RECONNECT_TIMEOUT) == 0)
    {
        APP_PRINT_INFO0("fmna_state_machine_handle_msg: fast reconnect adv change to slow");
        is_persistent_connection_disconnection = false;
        if (fmna_connection_get_num_connections() < fmna_connection_get_max_connections())
        {
            APP_PRINT_INFO1("fmna_persistent_connection_disconnection_timeout_handler: primary key index 0x%x",
                            m_fmna_current_primary_key.index);
            fmna_adv_init_nearby(m_fmna_current_primary_key.public_key);
            fmna_adv_start_slow_adv();
        }
    }
    else if (strcmp(topic, EVENT_BUS_TOPIC_FINDMY_STATE_MACHINE_UT_START) == 0)
    {
        // Enough time has passed that we need to enable motion detection and play sound on movement.
        fmna_motion_detection_start();
    }
    else if (strcmp(topic, EVENT_BUS_TOPIC_FINDMY_STATE_MACHINE_MT) == 0)
    {
        motion_detected_handler();
    }
    else
    {
        APP_PRINT_WARN0("fmna_state_machine_handle_msg: Invalid message type");
    }
}
