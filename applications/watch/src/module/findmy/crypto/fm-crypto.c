...
int fm_crypto_ckg_init(fm_crypto_ckg_context_t ctx)
{
    ...
    ctx->p = (mbedtls_ecp_point *)os_mem_zalloc(RAM_TYPE_DATA_ON, sizeof(mbedtls_ecp_point));
    if (ctx->p == NULL)
    {
        rv = -1;
        goto cleanup;
                                 ...
                                 const mbedtls_ecp_group *dp)
{
    ...
    int rv = mbedtls_ecp_muladd((mbedtls_ecp_group *)dp, r, &m, s, &m, t);
    CHECK_RV_GOTO(rv, cleanup);
                         ...
                         byte sks[32])
{
     ...
     */
    mbedtls_mpi_write_binary(&ctx->p->MBEDTLS_PRIVATE(X), x, 28);
