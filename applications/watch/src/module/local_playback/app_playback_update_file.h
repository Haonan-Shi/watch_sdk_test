...
/*ota version num*/
#define BLE_PLAYBACK_VERSION     0x4 // version:2 for BLE A B pingpong buf;version:3 for unicode; version:4 for utf8
#define SPP_PLAYBACK_VERSION     0x5
/*ic type*/
#define PLAYBACK_RTL87x3B              0x04
#define PLAYBACK_RTL87x3C             0x07
/*query info*/
...
#define PLAYBACK_PROTOCOL_VERSION   4 // version:1 for A B pingpong buf;version:3 for unicode; version:4 for utf8
...
#define PLAYBACK_LENGTH_TRIGGER_ROLE_SWAP   0
#define PLAYBACK_LENGTH_TRANS_CANCEL        0
#define PLAYBACK_LENGTH_DELETE_SONG         2
#define PLAYBACK_LENGTH_DELETE_ALL_SONG     0
#define PLAYBACK_LENGTH_GET_SD_SPACE_INFO   0
#define PLAYBACK_LENGTH_GET_FLASH_SPACE_INFO 0
#define PLAYBACK_LENGTH_DELETE_ALL_BY_FORMAT 1
...
typedef struct
{
    uint8_t result;             //byte0
    uint32_t sd_total_space;    //byte1-byte4
    uint32_t sd_free_space;     //byte5-byte8
} __attribute__((packed)) PLAYBACK_SD_SPACE_INFO;

typedef struct
{
    uint8_t result;                //byte0
    uint32_t flash_total_space;    //byte1-byte4
    uint32_t flash_free_space;     //byte5-byte8
} __attribute__((packed)) PLAYBACK_FLASH_SPACE_INFO;
...
typedef enum
{
    PLAYBACK_TRANS_REMOTE_CHARSET_UTF8,
    PLAYBACK_TRANS_REMOTE_CHARSET_UTF16,
} T_PB_TRANS_REMOTE_CHARSET;
...
typedef enum
{
    PB_TRANS_NORMAL = 0x00,
    PB_TRANS_CANCEL = 0x01,
} T_PB_TRANS_TERMINATION_REASON;

typedef enum
{
    PB_TRANS_SCENARIO_WATCH_GENERAL_FILE_TRANS  = 0x100,
    PB_TRANS_SCENARIO_WATCH_ONE_TIME_FILE       = 0x101,
    PB_TRANS_SCENARIO_WATCH_PICTURE_RESOURCE    = 0x102,
} T_PB_TRANS_SCENARIO;
