...
#define MAX_MALLOC  FF_MAX_SS  /* Must be >=FF_MAX_SS */
    ...
    if (sect != fs->winsect)    /* Window offset changed? */
    {
        ...
        if (res == FR_OK)           /* Fill sector window with new data */
        {
            if (disk_read(fs->pdrv, fs->win, sect, 1) != RES_OK)
            {
                ...
                APP_PRINT_ERROR1("fatfs: read data is not valid, line:%d", __LINE__);
                res = FR_DISK_ERR;
            }
            fs->winsect = sect;
        }
...
)
{
    ...
    do
    {
        ...
        else                    /* End of contiguous cluster block */
        {
            ...
            if (disk_ioctl(fs->pdrv, CTRL_TRIM, rt) != RES_OK) { return FR_DISK_ERR; }
#endif
            scl = ecl = nxt;
        }
#endif
...
)
{
    ...
    if (ofs >= (DWORD)((FF_FS_EXFAT &&
                        fs->fs_type == FS_EXFAT) ? MAX_DIR_EX : MAX_DIR)) { dp->sect = 0; }  /* Disable it if the offset reached the max value */
...
static UINT
check_fs(  /* 0:FAT VBR, 1:exFAT VBR, 2:Not FAT and valid BS, 3:Not FAT and invalid BS, 4:Disk error */
    FATFS *fs,          /* Filesystem object */
...
)
{
    ...
    {
        APP_PRINT_ERROR1("fatfs: the result of move_window is error, fmt = 4, line:%d", __LINE__);
        return 4;
    }   /* Load the boot sector */
    sign = ld_word(fs->win + BS_55AA);
#if FF_FS_EXFAT
    if (sign == 0xAA55 &&
        !mem_cmp(fs->win + BS_JmpBoot, "\xEB\x76\x90" "EXFAT   ", 11))   /* It is an exFAT VBR */
    {
        return 1;
    }
#endif
    b = fs->win[BS_JmpBoot];
    APP_PRINT_INFO2("fatfs: sign:0x%x, b:0x%x", sign, b);
    if (b == 0xEB || b == 0xE9 ||
        b == 0xE8)    /* Valid JumpBoot code? (short jump, near jump or near call) */
    {
        if (sign == 0xAA55 && !mem_cmp(fs->win + BS_FilSysType32, "FAT32   ", 8))  /* It is an FAT32 VBR */
        {
            return 0;
        }
...
)
{
    ...
    {
        APP_PRINT_INFO2("fatfs: it is a FAT VBR as auto scan, not a BS or disk error, fmt:%d, part:%d", fmt,
                        part);
        return fmt;
    ...
    if (fs->win[MBR_Table + PTE_System] == 0xEE)    /* GPT protective MBR? */
    {
        ...
        if (move_window(fs, 1) != FR_OK)  /* Load GPT header sector (next to MBR) */
        {
            return 4;
        }
        if (!test_gpt_header(fs->win))     /* Check if GPT header is valid */
        {
            return 3;
        }
        ...
        {
            if (move_window(fs, pt_lba + i * SZ_GPTE / SS(fs)) != FR_OK)   /* PT sector */
            {
                return 4;
            }
            ...
            if (!mem_cmp(fs->win + ofs + GPTE_PtGuid, GUID_MS_Basic, 16))   /* MS basic data partition? */
            {
                ...
                if (part == 0 && fmt <= 1)          /* Auto search (valid FAT volume found first) */
                {
                    return fmt;
                }
                if (part != 0 && v_ent == part)     /* Forced partition order (regardless of it is valid or not) */
                {
                    return fmt;
                }
            }
...
#endif
    if (FF_MULTI_PARTITION && part > 4)    /* MBR has 4 partitions max */
    {
        APP_PRINT_ERROR1("fatfs: MBR has more than 4 partitions, fmt = 3, line:%d", __LINE__);
        return 3;
    }
...
)
{
    ...
    *rfs = 0;
    ...
    if (vol < 0)
    {
        APP_PRINT_ERROR1("fatfs: logical drive number is invalid, line: %d", __LINE__);
    ...
    {
        APP_PRINT_ERROR2("fatfs: the file system object is not available, vol:%d line:%d", vol, __LINE__);
    ...
    *rfs = fs;                          /* Return pointer to the filesystem object */
    ...
    {
        APP_PRINT_ERROR1("fatfs: Check if the initialization succeeded, line:%d", __LINE__);
    ...
    if (fmt >= 2)
    {
        APP_PRINT_ERROR1("fatfs: No FAT volume is found,line: %d", __LINE__);
...
#endif  /* FF_FS_EXFAT */
    {
        ...
        {
            APP_PRINT_ERROR1("fatfs: BPB_BytsPerSec must be equal to the physical sector size, line:%d",
                             __LINE__);
            return FR_NO_FILESYSTEM;
        ...
        {
            APP_PRINT_ERROR1("fatfs: Number of FAT copies must be 1 or 2, line:%d", __LINE__);
            return FR_NO_FILESYSTEM;
        }    /* (Must be 1 or 2) */
        ...
        {
            APP_PRINT_ERROR1("fatfs: Number of sectors per cluster must be power of 2, line:%d", __LINE__);
            return FR_NO_FILESYSTEM;
        }   /* (Must be power of 2) */
        ...
        {
            APP_PRINT_ERROR1("fatfs: must be sector aligned, line:%d", __LINE__);
            return FR_NO_FILESYSTEM;
        ...
        if (nrsv == 0)
        {
            APP_PRINT_ERROR1("fatfs: Number of reserved sectors must not be 0, line:%d", __LINE__);
            return FR_NO_FILESYSTEM;
        }         /* (Must not be 0) */
        ...
        if (tsect < sysect)
        {
            APP_PRINT_ERROR1("fatfs: Invalid volume size, line:%d", __LINE__);
            return FR_NO_FILESYSTEM;
        }    /* (Invalid volume size) */
        ...
        if (fmt == 0)
        {
            APP_PRINT_ERROR1("fatfs: Invalid fatfs format, line:%d", __LINE__);
            return FR_NO_FILESYSTEM;
        }
        ...
        if (fmt == FS_FAT32)
        {
            ...
            {
                APP_PRINT_ERROR1("fatfs: Must be FAT32 revision 0.0, line:%d", __LINE__);
                return FR_NO_FILESYSTEM;
            }   /* (Must be FAT32 revision 0.0) */
            if (fs->n_rootdir != 0)
            {
                APP_PRINT_ERROR1("fatfs: BPB_RootEntCnt must be 0, line:%d", __LINE__);
                return FR_NO_FILESYSTEM;
        ...
        else
        {
            if (fs->n_rootdir == 0)
            {
                APP_PRINT_ERROR1("fatfs: BPB_RootEntCnt must not be 0, line:%d", __LINE__);
                return FR_NO_FILESYSTEM;
        ...
        {
            APP_PRINT_ERROR1("fatfs: BPB_FATSz must not be less than needed, line:%d", __LINE__);
            return FR_NO_FILESYSTEM;
...
)
{
    ...
    if (obj && obj->fs && obj->fs->fs_type && obj->id == obj->fs->id)
    {
        ...
        {
            DSTATUS stat;
            APP_PRINT_INFO1("fatfs: reinit once, line: %d", __LINE__);
            stat = disk_initialize(obj->fs->pdrv);   /* Initialize the physical drive */
            if (!(stat & STA_NOINIT))              /* Check if the initialization succeeded */
            {
                res = FR_OK;
            }
            ...
            {
                APP_PRINT_ERROR1("fatfs: physical drive init fail, line:%d", __LINE__);
                res = FR_NOT_READY;/* Failed to initialize due to no medium or hard error */
            }
        }
#endif
    }
...
)
{
    ...
    if (res == FR_OK)
    {
        *fatfs = fs;                /* Return ptr to the fs object */
        ...
        if ((g_fullFatScan == 0) && (fs->free_clst <= fs->n_fatent - 2))
        {
            *nclst = fs->free_clst;
        }
        else
            ...
            else
            {
...
#endif
                {
                    ...
                    do      /* Counts numbuer of entries with zero in the FAT */
                    {
                        ...
                        {
                            j = 0;
                            WDG_Kick();//Avoid taking too long to trigger the watch dog
                        }
                    }
                    while (--clst);
                }
...
)
{
    ...
    DIR djo = {0}, djn = {0};
    FATFS *fs = {0};
    BYTE buf[FF_FS_EXFAT ? SZDIRE * 2 : SZDIRE] = {0}, *dir = 0;
    LBA_t sect = 0;
    DEF_NAMBUF

    mem_set(&djo, 0, sizeof(DIR));

    if (get_ldnumber(&path_new) != -1)      /* Snip the drive number of new name off */
    {
...
)
{
...
#endif
    {
        ...
        // if (n_hd == 0) { n_hd = 254; }  /* Number of heads needs to be <256 */
...
)
{
    ...
    if (sz_vol < 128)
    {
        APP_PRINT_ERROR2("fatfs: sz_vol:%d, line:%d", sz_vol, __LINE__);
    ...
    if (fsty == FS_EXFAT)   /* Create an exFAT volume */
    {
        ...
        if (disk_ioctl(pdrv, CTRL_TRIM, lba) != RES_OK) { return FR_DISK_ERR; }
#endif
...
#endif  /* FF_FS_EXFAT */
    {
        ...
        do
        {
            ...
            if (fsty == FS_FAT32)   /* FAT32 volume */
            {
                ...
                {
                    APP_PRINT_ERROR2("fatfs: n_clst:0x%x, line:%d", n_clst, __LINE__);
                    LEAVE_MKFS(FR_MKFS_ABORTED);
                }
            }
            else                    /* FAT volume */
            ...
            {
                APP_PRINT_ERROR2("fatfs: sz_vol:0x%x, line:%d", sz_vol, __LINE__);
                LEAVE_MKFS(FR_MKFS_ABORTED);
            }  /* Too small volume? */
        ...
        if (fsty == FS_FAT32)
        {
            ...
            if (disk_write(pdrv, buf, b_vol + 6, 1) != RES_OK) { return FR_DISK_ERR; }
            mem_set(buf, 0, ss);
            ...
            if (disk_write(pdrv, buf, b_vol + 7, 1) != RES_OK) { return FR_DISK_ERR; }
            /* Write original FSINFO (VBR + 1) */
            if (disk_write(pdrv, buf, b_vol + 1, 1) != RES_OK) { return FR_DISK_ERR; }
        }
...
)
{
    ...
    for (;;)
    {
        ...
        while ((i < 32) && v && i < sizeof str / sizeof * str);
        if (f & 8) { str[i++] = '-'; }
        j = i; d = (f & 1) ? '0' : ' ';
        if (!(f & 2))
        {
