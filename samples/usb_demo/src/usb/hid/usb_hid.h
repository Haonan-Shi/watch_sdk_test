 ...
 * usb_hid.h
 *
 * \brief   HID mouse parameter
 *          \ref button: the last 3 bit if for button, the reset is device-specific
 *          \ref x: X displacement
 *          \ref y: Y displacement
 *
 */
typedef struct _hid_mouse_param
{
    uint8_t button;
    int8_t x;  // -127 ~ 127
    int8_t y;  // -127 ~ 127
    ...
    int8_t rsvd4;
    int8_t rsvd5;
} __attribute__((packed)) USB_HID_MOUSE_INPUT_REPORT;
 ...
 * usb_hid.h
 *
 * \brief   hid pipe send data specially for mouse
 *
 * \details data is sent serially, which means data is not sent actually until previous data transmission is complete.
 *
 * \param[in]  handle return value of \ref usb_hid_data_pipe_open
 * \param[in]  report_id report id of mouse
 * \param[in]  mouse_param mouse parameter defined in \ref USB_HID_MOUSE_INPUT_REPORT
 * \param[in]  len length of data
 *
 * \return true data will be sent now or after all previous transmissions are complete, false will never be sent
 *
