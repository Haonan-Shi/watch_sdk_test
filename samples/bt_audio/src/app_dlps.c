...
RAM_TEXT_SECTION void app_dlps_enable(uint32_t bit)
{
    if (dlps_bitmap & bit)
    {
        DBG_DIRECT("app_dlps_enable: %08x %08x -> %08x", bit, dlps_bitmap,
                   (dlps_bitmap & ~bit));
    }
    dlps_bitmap &= ~bit;
}
...
RAM_TEXT_SECTION void app_dlps_disable(uint32_t bit)
{
    ...
    {
        DBG_DIRECT("app_dlps_disable: %08x %08x -> %08x", bit, dlps_bitmap,
                   (dlps_bitmap | bit));
    }
...
{
    DBG_DIRECT("dump aon reg. 0x126 = 0x%x, 0x128 = 0x%x, 0x130 = 0x%x, 0x132 = 0x%x",
