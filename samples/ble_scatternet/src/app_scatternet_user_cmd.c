...
static T_USER_CMD_PARSE_RESULT cmd_showcon(T_USER_CMD_PARSED_VALUE *p_parse_value)
{
    ...
    for (conn_id = 0; conn_id < APP_MAX_LINKS; conn_id++)
    {
        ...
        {
            app_shell_print("ShowCon conn_id %d state 0x%x role %d\r\n", conn_id,
                            conn_info.conn_state, conn_info.role);
            app_shell_print("RemoteBd = [%02x:%02x:%02x:%02x:%02x:%02x] type = %d\r\n",
    ...
    app_shell_print("active link num %d,  idle link num %d\r\n",
...
static T_USER_CMD_PARSE_RESULT cmd_bondinfo(T_USER_CMD_PARSED_VALUE *p_parse_value)
{
    ...
    for (i = 0; i < bond_storage_num; i++)
    {
        ...
        if (p_entry != NULL)
        {
            app_shell_print("bond_dev[%d]: bd 0x%02x%02x%02x%02x%02x%02x, addr_type %d, flags 0x%x\r\n",
                            p_entry->idx,
...
{
    uint8_t i;
    app_shell_print("Advertising and Scan response: filter uuid = 0xA00A dev list\r\n");
    for (i = 0; i < dev_list_count; i++)
    {
        app_shell_print("RemoteBd[%d] = [%02x:%02x:%02x:%02x:%02x:%02x] type = %d\r\n",
                        i,
        ...
        if (dev_list[i].bd_type == GAP_REMOTE_ADDR_LE_RANDOM)
        {
            ...
            {
                app_shell_print("Static Random Addr\r\n");
            }
            else if (addr == RANDOM_ADDR_MASK_RESOLVABLE)
            {
                app_shell_print("Resolv Random Addr\r\n");
            }
            else if (addr == RANDOM_ADDR_MASK_NON_RESOLVABLE)
            {
                app_shell_print("Non-resolv Random Addr\r\n");
            }
            else
            {
                app_shell_print("Unknown Random Addr\r\n");
            }
        }
    }
...
static T_USER_CMD_PARSE_RESULT cmd_gaphdl(T_USER_CMD_PARSED_VALUE *p_parse_value)
{
    ...
    if (ret)
    {
        ...
        {
            app_shell_print("-->Index %d -- Handle 0x%x\r\n", hdl_idx, hdl_cache[hdl_idx]);
        }
        return (RESULT_SUCCESS);
    }
    else
